...
#define KeyBoradUID  "{4d1e55b2-f16f-11cf-88cb-001111000030}"

SECURITY_ATTRIBUTES		SA;

HANDLE HID_OpenForRead( CString strFileName )
{
...
HANDLE HID_OpenForWrite( CString strFileName )
{
	// Open a device that has already been identified as present
...
void HID_ErrorExit( CString strReason )
{
...
CString HID_FindHIDInterface( int nVID, int nPID, int nUsagePage )
{
/*
...
//	DWORD					dwBytesReturned, dwTemp;
	...
	if (hPnPHandle == INVALID_HANDLE_VALUE)
		HID_ErrorExit ("Could not attach to PnP node");
	...
	{	
		...
		if (!bSuccess)
		{
			...
			HID_ErrorExit ("Could not get the name of this HID device");
			SetupDiDestroyDeviceInfoList ( hPnPHandle );
			return strNameMatch;
		}

		strHidName = DeviceInterfaceDetailData->DevicePath;
		//OutputDebugString(strHidName);
		//if (-1 != strHidName.Find(KeyBoradUID))
		//{
		...
		{
			free(DeviceInterfaceDetailData);
			HID_ErrorExit ("Could not open HID device");
			SetupDiDestroyDeviceInfoList ( hPnPHandle );
			return strNameMatch;
		}
