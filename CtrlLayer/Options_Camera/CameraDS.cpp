...
/*
	...
	hr = m_pDeviceFilter->QueryInterface(IID_IAMVideoProcAmp, (void**)&pVideoProcAmp);
	if(SUCCEEDED(hr))
	{
		long Min, Max, Step, Default, Flags;
		hr = pVideoProcAmp->GetRange(VideoProcAmp_Brightness, &Min, &Max, &Step, &Default, &Flags);
		if(SUCCEEDED(hr))
		{
			long CurrentBrightness;
			hr = pVideoProcAmp->Get(VideoProcAmp_Brightness, &CurrentBrightness, &Flags);
			if(SUCCEEDED(hr))
			{
				printf("Current Brightness: %ld\n", CurrentBrightness);
			}

// 			hr = pVideoProcAmp->Set(VideoProcAmp_Brightness, Min, VideoProcAmp_Flags_Manual);
// 			if(SUCCEEDED(hr))
// 			{
// 				printf("Brightness set to minimum.\n");
// 			}
		}
...
IplImage* CCameraDS::QueryFrame()
{
	...
	HRESULT hr = m_pSampleGrabber->GetCurrentBuffer(&size, NULL);
	if (size == 0)
	{
		return NULL;
	}
