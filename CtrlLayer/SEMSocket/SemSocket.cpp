...
/*	if (nErrorCode == 0)
	{
		...
		PostMessage(m_destHandle, WM_USER_KYKYRM_Thread_Send_Connect, 0, LPARAM(pThread));   //发送连接线程的句柄给主程序
...
//		pThread->m_thrdIndex = m_pSerDlg->m_thrdIndex++;
		...
		connectSocket.Close();
		pThread->PostThreadMessage(WM_USER_KYKYRM_Thread_Send, KYKYRM_CMD_Sys_Connect_Success, 0);
	}
*/
...
void CSEMSocket::OnReceive(int nErrorCode)
{
	...
	if (m_pThrd)
	{
		m_pThrd->PostThreadMessage(WM_USER_KYKYRM_Thread_Read, 0, 0);
	}
}
