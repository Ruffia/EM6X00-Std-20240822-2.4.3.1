...
void CPanelStage::List_Init()
{
	...
	m_plistMC6600Pos	= new ListMC6600Pos;
...
void CPanelStage::List_Load()
{
    ...
    m_nPosTotal = GetPrivateProfileInt( "StageMC6600Pos", "Total", 0, path );
	if( m_nPosTotal == 0 )
		return;
...
void CPanelStage::List_ChangeName( int nItem, CString strName )
{
	...
	// 找到指定位置并从列表中删除
	int nCounts = 0;
	POSITION pos = m_plistMC6600Pos->GetHeadPosition();
	while( pos )
	{
		structMC6600Pos* pPos = m_plistMC6600Pos->GetAt( pos );
		if( nCounts == nItem && pPos->strName == strOldName )
		{
			pPos->strName = strName;
			break;
		}
		m_plistMC6600Pos->GetNext( pos );
		nCounts++;
	}
}
...
void CPanelStage::OnBnClickedButtonListdel()
{
	...
	// 找到指定位置并从列表中删除
	pos = m_plistMC6600Pos->GetHeadPosition();
	while( pos )
	{
		structMC6600Pos* pPos = m_plistMC6600Pos->GetAt( pos );
		if( pPos->strName == str )
		{
			m_plistMC6600Pos->RemoveAt( pos );
			delete pPos;
			break;
		}
		m_plistMC6600Pos->GetNext( pos );
	...
	m_nPosTotal = m_plistMC6600Pos->GetCount();
	List_Update();
}
...
void CPanelStage::List_Update()
{
	...
	CString str;
	str.Format( "%d", nTotal );
	WritePrivateProfileString( "StageMC6600Pos", "Total", str, path );  //将CString型变量存入Cfg.ini文件里，按照字段名进行检索（StageMC6600Pos）
	...
	while( pos )
	{
		structMC6600Pos* pPos = m_plistMC6600Pos->GetNext( pos );
...
*/
		...
		WritePrivateProfileString( "StageMC6600Pos", str, strPos, path );
	}
}
