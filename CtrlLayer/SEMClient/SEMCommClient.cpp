...
extern KYKYCommMsg_ImgFrameData g_msgImageFrameData;//[MALLOC_SIZE];
extern KYKYCommMsg_ImgData g_msgImageData[MALLOC_SIZE];
extern int g_nRecvLastIndex;
#define nSleepTime	20
...
*/
	Sleep(1000);
	m_pThreadImg = (CCSImgThread*)AfxBeginThread(RUNTIME_CLASS(CCSImgThread),THREAD_PRIORITY_NORMAL,0,CREATE_SUSPENDED);
	m_pThreadImg->SetServerIP(m_strIPAddr);
	m_pThreadImg->ResumeThread();
	return true;
}
...
void CSEMCommClient::Disconnect(bool bActive)   //主动/被动断开
{
	...
	if (!m_pThreadImg)  return;
	if (bActive)
	{
		m_pThreadImg->PostThreadMessage(WM_USER_KYKYComm_ClientThreadSend_Command, 0, KYKYComm_CMD_Sys_Disconnect);
		TRACE("Disconnect img active.\r\n");
		Sleep(1000);
	}

	m_pThreadImg->PostThreadMessage(WM_QUIT, 0, 0);
	m_pThreadImg = NULL;
	TRACE("delete imgThread\r\n");
...
/////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////
LPVOID CSEMCommClient::Img_RequestFrame()
{
	//开始请求发送图像
	TCP_SendCommand(KYKYComm_CMD_Image_GetFrameData);
	Sleep(25);
	return (&g_msgImageFrameData);
}

LPVOID CSEMCommClient::Img_Request()
{
	//开始请求发送图像
...
bool CSEMCommClient::StageGetDoubleParam2(int index, float* param)
{
	if( m_pThreadCmd != NULL )
	{
		TCP_SendCommand(index);
//		Sleep(nSleepTime);
		return m_pThreadCmd->StageGetDoubleParam2( index, param );
	}
	return false;
}

bool CSEMCommClient::StageGetDoubleParam5(int index, float* param)
{
	if( m_pThreadCmd != NULL )
	{
		TCP_SendCommand(index);
//		Sleep(nSleepTime);
		return m_pThreadCmd->StageGetDoubleParam5( index, param );
	}
	return false;
}
