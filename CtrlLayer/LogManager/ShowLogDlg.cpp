...
HCURSOR CShowLogDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

void CShowLogDlg::OnUpdateEditCols() 
{
...
void CShowLogDlg::OnUpdateEditFixcols() 
{
	if (!::IsWindow(m_Grid.m_hWnd)) return;
	UpdateData();
...
void CShowLogDlg::OnUpdateEditFixrows() 
{
	if (!::IsWindow(m_Grid.m_hWnd)) return;
	UpdateData();
...
void CShowLogDlg::OnUpdateEditRows() 
{	
	if (!::IsWindow(m_Grid.m_hWnd)) return;
	UpdateData();
...
void CShowLogDlg::OnVertGridLines() 
{
    m_bVertLines = !m_bVertLines;
    OnGridLines();
}

void CShowLogDlg::OnHorzGridLines() 
{
    m_bHorzLines = !m_bHorzLines;
    OnGridLines();
}

void CShowLogDlg::OnListmode() 
{
...
void CShowLogDlg::OnSingleselmode() 
{
...
void CShowLogDlg::OnSingleColMode() 
{
...
void CShowLogDlg::OnAllowSelection() 
{
...
void CShowLogDlg::OnPrintButton() 
{
...
static BOOL CALLBACK EnumProc(HWND hwnd, LPARAM lParam)
{
	...
	CShowLogDlg* pDlg = (CShowLogDlg*) pWnd->GetParent();
...
void CShowLogDlg::OnSize(UINT nType, int cx, int cy) 
{
...
#ifndef GRIDCONTROL_NO_CLIPBOARD
void CShowLogDlg::OnEditCopy() 
{
	m_Grid.OnEditCopy();	
}

void CShowLogDlg::OnEditCut() 
{
	m_Grid.OnEditCut();	
}

void CShowLogDlg::OnEditPaste() 
{
	m_Grid.OnEditPaste();	
}

void CShowLogDlg::OnUpdateEditCopyOrCut(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(m_Grid.GetSelectedCount() > 0);
}

void CShowLogDlg::OnUpdateEditPaste(CCmdUI* pCmdUI) 
{
...
void CShowLogDlg::OnEditSelectall() 
{
	m_Grid.OnEditSelectAll();
}

void CShowLogDlg::OnItalics() 
{
...
BOOL CShowLogDlg::PreTranslateMessage(MSG* pMsg) 
{
...
// NM_RCLICK
void CShowLogDlg::OnGridRClick(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
...
// GVN_BEGINLABELEDIT
void CShowLogDlg::OnGridStartEdit(NMHDR *pNotifyStruct, LRESULT* pResult)
{
    NM_GRIDVIEW* pItem = (NM_GRIDVIEW*) pNotifyStruct;
...
// GVN_ENDLABELEDIT
void CShowLogDlg::OnGridEndEdit(NMHDR *pNotifyStruct, LRESULT* pResult)
{
    NM_GRIDVIEW* pItem = (NM_GRIDVIEW*) pNotifyStruct;
...
// GVN_SELCHANGING
void CShowLogDlg::OnGridStartSelChange(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
    NM_GRIDVIEW* pItem = (NM_GRIDVIEW*) pNotifyStruct;
...
// GVN_SELCHANGED
void CShowLogDlg::OnGridEndSelChange(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
    NM_GRIDVIEW* pItem = (NM_GRIDVIEW*) pNotifyStruct;
...
// GVN_BEGINDRAG
void CShowLogDlg::OnGridBeginDrag(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
    NM_GRIDVIEW* pItem = (NM_GRIDVIEW*) pNotifyStruct;
    Trace(_T("Begin Drag row %d, col %d\n"), pItem->iRow, pItem->iColumn);
}

void CShowLogDlg::OnGridDblClick(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
    NM_GRIDVIEW* pItem = (NM_GRIDVIEW*) pNotifyStruct;
    Trace(_T("Double Clicked on row %d, col %d\n"), pItem->iRow, pItem->iColumn);
}

void CShowLogDlg::OnGridClick(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
    NM_GRIDVIEW* pItem = (NM_GRIDVIEW*) pNotifyStruct;
    Trace(_T("Clicked on row %d, col %d\n"), pItem->iRow, pItem->iColumn);
}

void CShowLogDlg::OnExpandToFit() 
{
    m_Grid.ExpandToFit(m_bExpandUseFixed);
}

void CShowLogDlg::OnAutoSize() 
{
    m_Grid.AutoSize();
}

void CShowLogDlg::OnFill() 
{
    m_Grid.DeleteAllItems();
    m_Grid.AutoFill();
}

void CShowLogDlg::OnCellNormal() 
{
    m_Grid.SetCellType(1,1, RUNTIME_CLASS(CGridCell));
    m_Grid.SetItemText(1,1, "1");
    m_Grid.SetItemState(1,1, m_Grid.GetItemState(1,1) & ~GVIS_READONLY);
    m_Grid.Invalidate();
}

void CShowLogDlg::OnCellReadonly() 
{
    OnCellNormal();
...
void CShowLogDlg::OnCellCheckbox()
{
    OnCellNormal();
...
void CShowLogDlg::OnCellDateTime()
{
    OnCellNormal();
...
void CShowLogDlg::OnCellUrl() 
{
    OnCellNormal();

    if (!m_Grid.SetCellType(1,1, RUNTIME_CLASS(CGridURLCell)))
        return;
    m_Grid.SetItemText(1,1, "www.kyky.com.cn");
}

void CShowLogDlg::OnCellCombo() 
{
    OnCellNormal();
    ...
    m_Grid.SetItemText(1,1, "Option 1");
...
void CShowLogDlg::OnCellNumeric() 
{
    OnCellNormal();
    ...
    m_Grid.SetItemText(1,1, "12345");
}


void CShowLogDlg::OnTrackfocus() 
{
...
void CShowLogDlg::OnRejectEditAttempts() 
{
    m_bRejectEditAttempts = !m_bRejectEditAttempts;
    UpdateMenuUI();
}

void CShowLogDlg::OnRejectEditChanges() 
{
    m_bRejectEditChanges = !m_bRejectEditChanges;
    UpdateMenuUI();
}

void CShowLogDlg::OnExpandLast() 
{
    m_Grid.ExpandLastColumn();	
}

void CShowLogDlg::OnSetFocus11() 
{
    CCellID cell(1,1);
...
void CShowLogDlg::OnCallbackFunction() 
{
...
bool CShowLogDlg::VirtualCompare(int c1, int c2)
{
...
void CShowLogDlg::OnVirtualMode() 
{
    UpdateData();
...
BOOL CShowLogDlg::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
...
BOOL CALLBACK CShowLogDlg::GridCallback(GV_DISPINFO *pDispInfo, LPARAM /*lParam*/) 
{
    pDispInfo->item.strText.Format(_T("Callback %d,%d"),pDispInfo->item.row, pDispInfo->item.col);
    return TRUE;
}

void CShowLogDlg::UpdateMenuUI()
{
#ifndef _WIN32_WCE
...
void CShowLogDlg::OnContextMenu(CWnd* pWnd, CPoint point) 
{
...
void CShowLogDlg::Trace(LPCTSTR szFmt, ...)
{    
	CString str;
...
void CShowLogDlg::OnCleartrace() 
{
	m_TraceWnd.SetWindowText(_T(""));
}

void CShowLogDlg::OnHide2ndrowcolumn()
{
