// SemCtrl_ScanShiftX.cpp: implementation of the CSemCtrl_ScanShiftX class.
//
...
CSemCtrl_ScanShiftX::CSemCtrl_ScanShiftX()
{
	m_scfCtrl = DefaultCtrlFigures_ScanShiftX;

	m_dCaliOffset = 0.0;
	CSemBasisCtrl::ReadINI( INI_CaliOffset, INIIndex_ShiftX, &m_dCaliOffset );
}
...
BOOL CSemCtrl_ScanShiftX::Flush()
{
...
//	wCtl = (WORD)(abs(lCtlIndex - m_scfCtrl.varMax.lVal));
	...
	hostMSG.bMessageID		= (BYTE)Z80B_EMoveX;
...
//	wusbCtl = (WORD)(m_scfCtrl.varMax.lVal - lCtlIndex -m_dCaliOffset);			// -2048 - 2047 对应 4095 - 0
	...
	usb_hostMSG.bData[0]		= (BYTE)USB_CID_SCN_Xshift_Crossover;	// Serial Comm operand low byte (X/Y Stig DAC circuit address)
...
const CString& CSemCtrl_ScanShiftX::FormatText()
{
	// GetPos
	VARIANT varPos = GetPos();
	long lCtlIndex = varPos.lVal;
...
const CString& CSemCtrl_ScanShiftX::FormatText( VARIANT varPos )
{
...
void CSemCtrl_ScanShiftX::SetParams( int nType, double dParam )// Type:类型；Param:要设置的参数值
{
	switch( nType )
	{
	case	100:	// OprAdvance
