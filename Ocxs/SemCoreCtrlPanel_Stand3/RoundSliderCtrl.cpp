...
#pragma warning(default:4244)
	...
	if(!strFormattedText.IsEmpty())
	{
		...
		CSize szExtent = pDC->GetTextExtent(strFormattedText);
		CPoint ptText = CPoint(m_ptCenter.x - szExtent.cx/2, m_ptCenter.y - szExtent.cy/2);
		...
		{
			rc.DeflateRect(m_ptCenter.x - nRadius + 1,m_ptCenter.x - nRadius + 1);
			if( szExtent.cx > rc.Width() )	// 标签超长
			{
				CString strTemp, str = strFormattedText;
				// 计算标签中共有多少个空格符
				...
				str = strFormattedText;
				nLength = str.GetLength();
				// 计算分行显示时第一行的上边界值
				ptText.y = (int)(m_ptCenter.y -(nCount +1) /2.0 * szExtent.cy);
				// 分多行显示标签
				int i;
				for( i=0; i<=nCount; i++ )
				{
					...
					szExtent = dc.GetTextExtent( strTemp );
					// 调整本行的横向位置
					ptText.x = m_ptCenter.x - szExtent.cx / 2 +1;
					// 输出本行字符
					pDC->TextOut( ptText.x, ptText.y, strTemp );
					// 调整下一行的纵向位置
					ptText.y += szExtent.cy;
				}
			}
			else
...
CString CRoundSliderCtrl::OnFormatText() const
{
	...
	strFormattedText.Format(GetText(), GetPos() /10.0);
