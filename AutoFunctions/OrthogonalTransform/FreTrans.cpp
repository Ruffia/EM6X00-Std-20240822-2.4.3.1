	...
	double &dflWalsh )
{
	...
	/////////////////////////////////////////////
	// 列
	for(i = 0; i < lWalshHeight; i++)
	{
		// 行
		for(j = 0; j < lWalshWidth; j++)
		{
			dTemp = fabs(F[i * w + j] * 1000);
			dfIntegralWalsh += dTemp;
		}
	}
	/////////////////////////////////////////////
	...
	double &dfTotal )
{
	...
	long	Ix, Iy, nP, nS, nQ;
	for ( Iy = 1; Iy < lHeight; Iy++ )
	{
		for ( Ix = 1; Ix < lWidth; Ix++ )
		{
			nP = (BYTE)( lpDIBBits[  Iy    * lWidth +  Ix    ] );
			nS = (BYTE)( lpDIBBits[ (Iy-1) * lWidth +  Ix    ] );
			nQ = (BYTE)( lpDIBBits[  Iy    * lWidth + (Ix-1) ] );
