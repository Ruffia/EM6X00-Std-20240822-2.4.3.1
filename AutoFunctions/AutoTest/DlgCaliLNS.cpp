...
void CDlgCaliLNS::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( theApp.m_Cali.nCaliLNS_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = theApp.m_Cali.nCaliLNS_Total[nIndex];
	...
	{
		dYMin = (theApp.m_Cali.pCaliLNS[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.pCaliLNS[nIndex][i] < dYMin )
				dYMin = theApp.m_Cali.pCaliLNS[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (theApp.m_Cali.pCaliLNS[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.pCaliLNS[nIndex][i] > dYMax )
				dYMax = theApp.m_Cali.pCaliLNS[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.pCaliLNS[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= theApp.m_Cali.pCaliLNS[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.pCaliLNS[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= theApp.m_Cali.pCaliLNS[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
/*	// TODO: Add your control notification handler code here
	...
	for( int i=0; i<Cali_16bitDAC_Steps +1; i++ )
	{
		for( int j=0; j<4; j++ )
		{
			...
			str.Format( "%.6f", theApp.m_Cali.pCaliLNS[j][i]);
			sel.Move( &varOptional, &varOptional );
			sel.TypeText( str );
		}
...
void CDlgCaliLNS::EnableParams( int nIndex, BOOL bEnable )
{
	...
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.nCaliLNS_OldPos[0];
			Xpos		= var;
		...
		case	2:
			...
			Flag.lVal	= 1;
			var.dblVal	= theApp.m_Cali.nCaliLNS_OldPos[1];
		...
		case	4:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.nCaliLNS_OldPos[2];
			Xpos		= var;
		...
		case	6:
			...
			ClassID.lVal = SemControlPanelClass_BeemAlignment;
			Flag.lVal	= 2;
			var.lVal	= theApp.m_Cali.nCaliLNS_OldPos[3];
			Ypos		= var;
	...
	else
	{
		...
		str.Format( "%s\\Log\\AutoCaliLNS-%s.txt", appfilepath, strTemp );
		...
		sprintf( Buf, "Auto Calibration for LNS board --- %s\n", str );
...
void CDlgCaliLNS::OnStartOL() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliLNS[1][i] = 0;
		theApp.m_Cali.nCaliLNS_Total[1] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_ObjectiveLens.GetPos();
		theApp.m_Cali.nCaliLNS_OldPos[1] = var.lVal;
...
void CDlgCaliLNS::OnStartALX() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliLNS[2][i] = 0;
		theApp.m_Cali.nCaliLNS_Total[2] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_BeemAlignmentX.GetPos();
		theApp.m_Cali.nCaliLNS_OldPos[2] = var.lVal;
...
void CDlgCaliLNS::OnStartALY() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliLNS[3][i] = 0;
		theApp.m_Cali.nCaliLNS_Total[3] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_BeemAlignmentY.GetPos();
		theApp.m_Cali.nCaliLNS_OldPos[3] = var.lVal;
...
//	if( nIDEvent == Timer_Cali_CL )
	{
		...
		nPos = theApp.m_Cali.nCaliLNS_Total[0];
		theApp.m_Cali.pCaliLNS[0][nPos] = dValue;
		theApp.m_Cali.nCaliLNS_Total[0]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		...
		else
		{
			...
			if( nPos > g_n16bitDAC_Max -32768 )
				nPos = g_n16bitDAC_Max -32768;
			theApp.LNS_L1DACSB_Change( nPos );
		}
	}
}
...
//	if( nIDEvent == Timer_Cali_OL )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 5, dValue );
		nPos = theApp.m_Cali.nCaliLNS_Total[1];
		theApp.m_Cali.pCaliLNS[1][nPos] = dValue;
		theApp.m_Cali.nCaliLNS_Total[1]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_ALX )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 5, dValue );
		nPos = theApp.m_Cali.nCaliLNS_Total[2];
		theApp.m_Cali.pCaliLNS[2][nPos] = dValue;
		theApp.m_Cali.nCaliLNS_Total[2]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_ALY )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 5, dValue );
		nPos = theApp.m_Cali.nCaliLNS_Total[3];
		theApp.m_Cali.pCaliLNS[3][nPos] = dValue;
		theApp.m_Cali.nCaliLNS_Total[3]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
