...
void CDlgCaliSCN::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( theApp.m_Cali.nCaliSCN_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = theApp.m_Cali.nCaliSCN_Total[nIndex];
	...
	{
		dYMin = (theApp.m_Cali.pCaliSCN[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.pCaliSCN[nIndex][i] < dYMin )
				dYMin = theApp.m_Cali.pCaliSCN[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (theApp.m_Cali.pCaliSCN[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.pCaliSCN[nIndex][i] > dYMax )
				dYMax = theApp.m_Cali.pCaliSCN[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.pCaliSCN[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= theApp.m_Cali.pCaliSCN[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.pCaliSCN[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= theApp.m_Cali.pCaliSCN[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
/*	// TODO: Add your control notification handler code here
	...
	for( int i=0; i<Cali_12bitDAC_Steps +1; i++ )
	{
		for( int j=0; j<4; j++ )
		{
			...
			str.Format( "%.6f", theApp.m_Cali.pCaliSCN[j][i]);
			sel.Move( &varOptional, &varOptional );
			sel.TypeText( str );
		}
...
void CDlgCaliSCN::EnableParams( int nIndex, BOOL bEnable )
{
	...
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.nCaliSCN_OldPos[0];
			Xpos		= var;
		...
		case	2:
			...
			ClassID.lVal = SemControlPanelClass_Stigmator;
			Flag.lVal	= 2;
			var.lVal	= theApp.m_Cali.nCaliSCN_OldPos[1];
			Ypos		= var;
		...
		case	4:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.nCaliSCN_OldPos[2];
			Xpos		= var;
		...
		case	6:
			...
			ClassID.lVal = SemControlPanelClass_ScanShift;
			Flag.lVal	= 2;
			var.lVal	= theApp.m_Cali.nCaliSCN_OldPos[3];
			Ypos		= var;
	...
	else
	{
		...
		sprintf( Buf, "Auto Calibration for SCN board --- %s\n", str );
...
void CDlgCaliSCN::OnStartStigX() 
{
	...
	if( nCheck )
	{
		...
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliSCN[0][i] = 0;
		theApp.m_Cali.nCaliSCN_Total[0] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_StigmatorX.GetPos();
		theApp.m_Cali.nCaliSCN_OldPos[0] = var.lVal;

		int nPos = -2048;	// 设置起始值
		theApp.SCN_XstigDACSB_Change( nPos );
...
void CDlgCaliSCN::OnStartStigY() 
{
	...
	if( nCheck )
	{
		...
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliSCN[1][i] = 0;
		theApp.m_Cali.nCaliSCN_Total[1] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_StigmatorY.GetPos();
		theApp.m_Cali.nCaliSCN_OldPos[1] = var.lVal;

		int nPos = -2048;	// 设置起始值
		theApp.SCN_YstigDACSB_Change( nPos );
...
void CDlgCaliSCN::OnStartShiftX() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliSCN[2][i] = 0;
		theApp.m_Cali.nCaliSCN_Total[2] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_ScanShiftX.GetPos();
		theApp.m_Cali.nCaliSCN_OldPos[2] = var.lVal;
...
//		int nValue = 0x800;
		...
		int nPos = -2048;	// 设置起始值
		theApp.SCN_XshiftDACSB_Change( nPos );
...
void CDlgCaliSCN::OnStartShiftY() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliSCN[3][i] = 0;
		theApp.m_Cali.nCaliSCN_Total[3] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_ScanShiftY.GetPos();
		theApp.m_Cali.nCaliSCN_OldPos[3] = var.lVal;
...
//		int nValue = 0x800;
		...
		int nPos = -2048;	// 设置起始值
		theApp.SCN_YshiftDACSB_Change( nPos );
...
//	if( nIDEvent == Timer_Cali_StigX )
	{
		...
		nPos = theApp.m_Cali.nCaliSCN_Total[0];
		theApp.m_Cali.pCaliSCN[0][nPos] = dValue;
		theApp.m_Cali.nCaliSCN_Total[0]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
...
//	if( nIDEvent == Timer_Cali_StigY )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 80, dValue );
		nPos = theApp.m_Cali.nCaliSCN_Total[1];
		theApp.m_Cali.pCaliSCN[1][nPos] = dValue;
		theApp.m_Cali.nCaliSCN_Total[1]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_SCN_ShiftX )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 80, dValue );
		nPos = theApp.m_Cali.nCaliSCN_Total[2];
		theApp.m_Cali.pCaliSCN[2][nPos] = dValue;
		theApp.m_Cali.nCaliSCN_Total[2]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_SCN_ShiftY )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 80, dValue );
		nPos = theApp.m_Cali.nCaliSCN_Total[3];
		theApp.m_Cali.pCaliSCN[3][nPos] = dValue;
		theApp.m_Cali.nCaliSCN_Total[3]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
