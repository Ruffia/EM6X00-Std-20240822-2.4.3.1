...
void CDlgCaliSPR::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( theApp.m_Cali.nCaliSPR_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = theApp.m_Cali.nCaliSPR_Total[nIndex];
	...
	{
		dYMin = (theApp.m_Cali.pCaliSPR[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.pCaliSPR[nIndex][i] < dYMin )
				dYMin = theApp.m_Cali.pCaliSPR[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (theApp.m_Cali.pCaliSPR[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.pCaliSPR[nIndex][i] > dYMax )
				dYMax = theApp.m_Cali.pCaliSPR[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.pCaliSPR[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= theApp.m_Cali.pCaliSPR[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.pCaliSPR[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= theApp.m_Cali.pCaliSPR[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
// 	sel.TypeText( str );
	...
	for( int i=0; i<Cali_16bitDAC_Steps +1; i++ )
	{
		for( int j=0; j<1; j++ )
		{
			...
			str.Format( "%.6f", theApp.m_Cali.pCaliSPR[j][i]);
			sel.Move( &varOptional, &varOptional );
			sel.TypeText( str );
		}
...
void CDlgCaliSPR::EnableParams( int nIndex, BOOL bEnable )
{
	...
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.nCaliSPR_OldPos[0];
			Xpos		= var;
	...
	else
	{
		...
		unsigned long cbRet;
		char Buf[1024];
		sprintf( Buf, "Auto Calibration for SPR board --- Contrast\n" );
...
void CDlgCaliSPR::OnStartContrast() 
{
	...
	if( nCheck )
	{
		...
		for( i=0; i<nMax; i++ )
			theApp.m_Cali.pCaliSPR[0][i] = 0;
		theApp.m_Cali.nCaliSPR_Total[0] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_Contrast.GetPos();
		theApp.m_Cali.nCaliSPR_OldPos[0] = var.lVal;
...
//	if( nIDEvent == Timer_Cali_Contrast )
	{
		...
		nPos = theApp.m_Cali.nCaliSPR_Total[0];
		theApp.m_Cali.pCaliSPR[0][nPos] = dValue;
		theApp.m_Cali.nCaliSPR_Total[0]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
