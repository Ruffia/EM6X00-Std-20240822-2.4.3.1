...
void CDlgAutoTestLNS::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( theApp.m_AT.nAutoTestLNS_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = theApp.m_AT.nAutoTestLNS_Total[nIndex];
	...
	{
		dYMin = (theApp.m_AT.pAutoTestLNS[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_AT.pAutoTestLNS[nIndex][i] < dYMin )
				dYMin = theApp.m_AT.pAutoTestLNS[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (theApp.m_AT.pAutoTestLNS[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_AT.pAutoTestLNS[nIndex][i] > dYMax )
				dYMax = theApp.m_AT.pAutoTestLNS[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_AT.pAutoTestLNS[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= theApp.m_AT.pAutoTestLNS[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (theApp.m_AT.pAutoTestLNS[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= theApp.m_AT.pAutoTestLNS[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
//			pDC->Ellipse( CRect(x-2, yLog-2, x+2, yLog+2) );
	...
	// 标注纵向最小值
	noldBKmode = pDC->SetBkMode(TRANSPARENT);
	double dValue = theApp.GetPhysicsFromAdcs(1*16 +m_nChnIndex2, dYMin);
	str.Format( "%.6f ( %.6f )", dYMin, dValue );
	pDC->TextOut( rc1.left -8, rc1.bottom -sz.cy/2, str );
	// 标注纵向最大值
	dValue = theApp.GetPhysicsFromAdcs(1*16 +m_nChnIndex2, dYMax);
	...
	str.Format( "%s [%04X---%04X]", theApp.m_DL.adc.chn[1*16 +m_nChnIndex2].strLabel, pn[0], pn[AutoTest_16bitDAC_StepsEach-1] );
	sz = pDC->GetTextExtent( str );
...
{
	theApp.m_DL.other.bDebug = theApp.m_DL.other.bDebug;
	UpdateData( FALSE );
}
...
void CDlgAutoTestLNS::EnableParams( int nIndex, BOOL bEnable )
{
	...
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_AT.nAutoTestLNS_OldPos[0];
			Xpos		= var;
		...
		case	2:
			...
			Flag.lVal	= 1;
			var.dblVal	= theApp.m_AT.nAutoTestLNS_OldPos[1];
		...
		case	4:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_AT.nAutoTestLNS_OldPos[2];
			Xpos		= var;
		...
		case	6:
			...
			ClassID.lVal = SemControlPanelClass_BeemAlignment;
			Flag.lVal	= 2;
			var.lVal	= theApp.m_AT.nAutoTestLNS_OldPos[3];
			Ypos		= var;
	...
	else
	{
		...
		sprintf( Buf, "Auto test for LNS board --- %s\n", str );
...
{
	// TODO: Add your control notification handler code here
	int i, j, nCheck = ((CButton*)(GetDlgItem(IDC_CHECK_AT_Cali_Start)))->GetCheck();
	if( nCheck )
	{
		for( i=0; i<4; i++ )
		{
			theApp.m_AT.nAutoTestLNS_Total[i] = 0;
			for( j=0; j<AutoTest_16bitDAC_Steps; j++ )
				theApp.m_AT.pAutoTestLNS[i][j] = 0;
		}
		VARIANT var = CSemCoreAssemble::Instance().m_CondenserLens.GetPos();
		theApp.m_AT.nAutoTestLNS_OldPos[0] = var.lVal;
		var = CSemCoreAssemble::Instance().m_ObjectiveLens.GetPos();
		theApp.m_AT.nAutoTestLNS_OldPos[1] = var.lVal;
		var = CSemCoreAssemble::Instance().m_BeemAlignmentX.GetPos();
		theApp.m_AT.nAutoTestLNS_OldPos[2] = var.lVal;
		var = CSemCoreAssemble::Instance().m_BeemAlignmentY.GetPos();
		theApp.m_AT.nAutoTestLNS_OldPos[3] = var.lVal;
		...
		theApp.LNS_L1DACSB_Change( nPos );
...
void CDlgAutoTestLNS::OnStartCL() 
{
	...
	if( nCheck )
	{
		...
		for( i=0; i<nMax; i++ )
		{
			theApp.m_AT.pAutoTestLNS[0][i] = 0;
			theApp.m_AT.pAutoTestLNS[1][i] = 0;
			theApp.m_AT.pAutoTestLNS[2][i] = 0;
		}
		theApp.m_AT.nAutoTestLNS_Total[0] = 0;
		theApp.m_AT.nAutoTestLNS_Total[1] = 0;
		theApp.m_AT.nAutoTestLNS_Total[2] = 0;
...
void CDlgAutoTestLNS::OnStartOL() 
{
	...
	if( nCheck )
	{
		...
		int nPos = -32768 + g_nAutoTestLNS_seg1[m_nOrder];	// 设置起始值
		m_nOrder++;
		theApp.LNS_L2SDACSB_Change( nPos );
...
void CDlgAutoTestLNS::OnStartALX() 
{
	...
	if( nCheck )
	{
		...
		int nPos = -32768 + g_nAutoTestLNS_seg1[m_nOrder];	// 设置起始值
		m_nOrder++;
		theApp.LNS_XalDACSB_Change( nPos );
...
void CDlgAutoTestLNS::OnStartALY() 
{
	...
	if( nCheck )
	{
		...
		int nPos = -32768 + g_nAutoTestLNS_seg1[m_nOrder];	// 设置起始值
		m_nOrder++;
		theApp.LNS_YalDACSB_Change( nPos );
...
{
	// TODO: Add your control notification handler code here
	int i, nCheck = ((CButton*)(GetDlgItem(IDC_CHECK_AT_Cali_Start_22)))->GetCheck();
	if( nCheck )
	{
		// 暂停真空状态机的工作
		if( CSMvac::Instance().isReady() )
...
{
	// TODO: Add your control notification handler code here
	int i, nCheck = ((CButton*)(GetDlgItem(IDC_CHECK_AT_Cali_Start_32)))->GetCheck();
	if( nCheck )
	{
		// 暂停真空状态机的工作
		if( CSMvac::Instance().isReady() )
...
{
	// TODO: Add your control notification handler code here
	int i, nCheck = ((CButton*)(GetDlgItem(IDC_CHECK_AT_Cali_Start_42)))->GetCheck();
	if( nCheck )
	{
		// 暂停真空状态机的工作
		if( CSMvac::Instance().isReady() )
...
void CDlgAutoTestLNS::TimerChangeCL( UINT nIDEvent )
{
	...
	if( nIDEvent == Timer_CL_seg11 )
	{
		...
		if( theApp.m_DL.other.bDebug )
		{
			// 调试模式下，记录3次值
			dValue = 0.0;
			str = theApp.ReadSelectedADC_Logging_onetime( 5, dValue );
			nPos = theApp.m_AT.nAutoTestLNS_Total[0];
			theApp.m_AT.pAutoTestLNS[0][nPos] = dValue;
			theApp.m_AT.nAutoTestLNS_Total[0]++;
			m_strRes += str;

			dValue = 0.0;
			str = theApp.ReadSelectedADC_Logging_onetime( 5, dValue, TRUE );
			nPos = theApp.m_AT.nAutoTestLNS_Total[0];
			theApp.m_AT.pAutoTestLNS[0][nPos] = dValue;
			theApp.m_AT.nAutoTestLNS_Total[0]++;
			m_strRes += str;
	...
	else if( nIDEvent == Timer_CL_seg13 )
	{
		...
		if( theApp.m_DL.other.bDebug )
		{
			...
			dValue = 0.0;
			str = theApp.ReadSelectedADC_Logging_onetime( 5, dValue );
			nPos = theApp.m_AT.nAutoTestLNS_Total[1];
			theApp.m_AT.pAutoTestLNS[1][nPos] = dValue;
			theApp.m_AT.nAutoTestLNS_Total[1]++;
			m_strRes += str;
	...
	else if( nIDEvent == Timer_CL_seg15 )
	{
		...
		if( theApp.m_DL.other.bDebug )
		{
			...
			dValue = 0.0;
			str = theApp.ReadSelectedADC_Logging_onetime( 5, dValue );
			nPos = theApp.m_AT.nAutoTestLNS_Total[2];
			theApp.m_AT.pAutoTestLNS[2][nPos] = dValue;
			theApp.m_AT.nAutoTestLNS_Total[2]++;
			m_strRes += str;
