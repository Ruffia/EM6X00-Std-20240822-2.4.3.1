...
void CDlgCaliMCI::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( theApp.m_Cali.m_nCaliMCI_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = theApp.m_Cali.m_nCaliMCI_Total[nIndex];
	...
	{
		dYMin = (theApp.m_Cali.m_pCaliMCI[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.m_pCaliMCI[nIndex][i] < dYMin )
				dYMin = theApp.m_Cali.m_pCaliMCI[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (theApp.m_Cali.m_pCaliMCI[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.m_pCaliMCI[nIndex][i] > dYMax )
				dYMax = theApp.m_Cali.m_pCaliMCI[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.m_pCaliMCI[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= theApp.m_Cali.m_pCaliMCI[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.m_pCaliMCI[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= theApp.m_Cali.m_pCaliMCI[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
/*	for( int i=0; i<Cali_16bitDAC_Steps +1; i++ )
	{
		for( int j=0; j<4; j++ )
		{
			...
			str.Format( "%.6f", theApp.m_Cali.m_pCaliMCI[j][i]);
			sel.Move( &varOptional, &varOptional );
			sel.TypeText( str );
		}
...
void CDlgCaliMCI::InitParams()
{
	for( int i=0; i<4; i++ )
	{
		theApp.m_Cali.m_pCaliMCI[i] = new double[Cali_16bitDAC_Steps+1];
		for( int j=0; j<Cali_16bitDAC_Steps+1; j++ )
			theApp.m_Cali.m_pCaliMCI[i][j] = 0;
		theApp.m_Cali.m_nCaliMCI_Total[i] = 0;
		m_nChnIndex2[i] = 0;
	}
}
...
void CDlgCaliMCI::EnableParams( int nIndex, BOOL bEnable )
{
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.m_nCaliMCI_OldPos[0];
			Xpos		= var;
		...
		case	2:
			...
			ClassID.lVal = SemControlPanelClass_Stigmator;
			Flag.lVal	= 2;
			var.lVal	= theApp.m_Cali.m_nCaliMCI_OldPos[1];
			Ypos		= var;
		...
		case	4:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.m_nCaliMCI_OldPos[2];
			Xpos		= var;
		...
		case	6:
			...
			ClassID.lVal = SemControlPanelClass_ScanShift;
			Flag.lVal	= 2;
			var.lVal	= theApp.m_Cali.m_nCaliMCI_OldPos[3];
			Ypos		= var;
...
void CDlgCaliMCI::OnStartFineMagX() 
{
	...
	if( nCheck )
	{
		...
		for( int i=0; i<nMax; i++ )
			theApp.m_Cali.m_pCaliMCI[0][i] = 0;
		theApp.m_Cali.m_nCaliMCI_Total[0] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = theApp.m_SemCtrl.m_SemCore.m_StigmatorX.GetPos();
		theApp.m_Cali.m_nCaliMCI_OldPos[0] = var.lVal;
		...
		theApp.m_SemCtrl.USB_WriteToBoard( USB_SetXoffset, TRUE, 0 );	// Digital Shift X
...
void CDlgCaliMCI::OnStartFineMagY() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_12bitDAC_Steps;
		for( int i=0; i<nMax; i++ )
			theApp.m_Cali.m_pCaliMCI[1][i] = 0;
		theApp.m_Cali.m_nCaliMCI_Total[1] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = theApp.m_SemCtrl.m_SemCore.m_StigmatorY.GetPos();
		theApp.m_Cali.m_nCaliMCI_OldPos[1] = var.lVal;
		...
		theApp.m_SemCtrl.USB_WriteToBoard( USB_SetYoffset, TRUE, 0 );	// Digital Shift Y
...
void CDlgCaliMCI::OnStartShiftX() 
{
	...
	if( nCheck )
	{
		...
		for( int i=0; i<nMax; i++ )
			theApp.m_Cali.m_pCaliMCI[2][i] = 0;
		theApp.m_Cali.m_nCaliMCI_Total[2] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = theApp.m_SemCtrl.m_SemCore.m_ScanShiftX.GetPos();
		theApp.m_Cali.m_nCaliMCI_OldPos[2] = var.lVal;
...
void CDlgCaliMCI::OnStartShiftY() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( int i=0; i<nMax; i++ )
			theApp.m_Cali.m_pCaliMCI[3][i] = 0;
		theApp.m_Cali.m_nCaliMCI_Total[3] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = theApp.m_SemCtrl.m_SemCore.m_ScanShiftY.GetPos();
		theApp.m_Cali.m_nCaliMCI_OldPos[3] = var.lVal;
...
//	if( nIDEvent == Timer_Cali_FineMagX )
	{
		...
		nPos = theApp.m_Cali.m_nCaliMCI_Total[0];
		theApp.m_Cali.m_pCaliMCI[0][nPos] = dValue;
		theApp.m_Cali.m_nCaliMCI_Total[0]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
...
//	if( nIDEvent == Timer_Cali_FineMagY )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 80, dValue );
		nPos = theApp.m_Cali.m_nCaliMCI_Total[1];
		theApp.m_Cali.m_pCaliMCI[1][nPos] = dValue;
		theApp.m_Cali.m_nCaliMCI_Total[1]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_MCI_ShiftX )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 80, dValue );
		nPos = theApp.m_Cali.m_nCaliMCI_Total[2];
		theApp.m_Cali.m_pCaliMCI[2][nPos] = dValue;
		theApp.m_Cali.m_nCaliMCI_Total[2]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_MCI_ShiftY )
	{
		...
		dValue = 0.0;
		str = theApp.ReadSelectedADC_Logging2( 80, dValue );
		nPos = theApp.m_Cali.m_nCaliMCI_Total[3];
		theApp.m_Cali.m_pCaliMCI[3][nPos] = dValue;
		theApp.m_Cali.m_nCaliMCI_Total[3]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( theApp.m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
