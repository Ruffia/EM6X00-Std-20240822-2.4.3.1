...
HBRUSH CDlgAutoTestVAC::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	if( nCtlColor == CTLCOLOR_STATIC )
	{
		...
		if( nID == IDC_STATIC_AT_Cali_VAC_0_0 +nIndex
			&& nIndex >=0 && nIndex <= 22
			&& theApp.m_AT.bVAC_OutofRange[nIndex] )
		{
...
void CDlgAutoTestVAC::InitParams()
{
	...
	// VAC ADC#0:
	for( i=0; i<16; i++ )
		GetDlgItem( IDC_STATIC_AT_Cali_VAC_0_0 +i)->SetWindowText( theApp.m_DL.adc.chn[2*16+i].strLabel );
	/////////////////////////////////////////////////////////////////

	/////////////////////////////////////////////////////////////////
	// VAC ADC#1:
	for( i=0; i<6; i++ )
		GetDlgItem( IDC_STATIC_AT_Cali_VAC_1_0 +i)->SetWindowText( theApp.m_DL.adc.chn[3*16+i].strLabel );
	/////////////////////////////////////////////////////////////////
...
void CDlgAutoTestVAC::OnStart() 
{
	...
	for( i=0; i<16; i++ )
	{
		...
		theApp.m_AT.pAutoTestVAC[k] = dValue;
		str.Format( "%.6f", theApp.m_AT.pAutoTestVAC[i] );
		GetDlgItem( IDC_EDIT_AT_Cali_VAC_VAC00_Vadc + k )->SetWindowText( str );

		//////////////////////////
		// 显示ADC计算分析值
		//////////////////////////
		dValue = theApp.GetPhysicsFromAdcs( 2*16 +i, theApp.m_AT.pAutoTestVAC[k] );
	...
	for( i=0; i<6; i++ )
	{
		...
		// 显示ADC计算分析值
		//////////////////////////
		dValue = theApp.GetPhysicsFromAdcs( 3*16 +i, theApp.m_AT.pAutoTestVAC[k] );
		str.Format( "%.6f", dValue );
		GetDlgItem( IDC_EDIT_AT_Cali_VAC_VAC00_Vadc3 +k )->SetWindowText( str );
