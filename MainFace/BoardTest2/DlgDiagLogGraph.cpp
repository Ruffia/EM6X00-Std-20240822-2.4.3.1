...
BOOL CDlgDiagLogGraph::OnInitDialog() 
{
	...
	for( int i=0; i<GraphDataBuffGroups; i++ )
	{
		theApp.m_DL.graph[i].nZoomXIndex = 3;	// 横向显示128个数据
		theApp.m_DL.graph[i].nZoomYIndex = -1;

		theApp.m_DL.graph[i].dXMax = g_dGraphRangeX[3];
		theApp.m_DL.graph[i].dXMin = 0;

		theApp.m_DL.graph[i].dYMin = 0.0;
		theApp.m_DL.graph[i].dYMax = 0.0;
	}
	m_scroll1.SetScrollRange( 0, (int)(theApp.m_DL.graph[0].dXMax) );
	m_scroll1.ShowWindow( TRUE );
	m_scroll2.SetScrollRange( 0, (int)(theApp.m_DL.graph[1].dXMax) );
	m_scroll2.ShowWindow( TRUE );
	m_scroll3.SetScrollRange( 0, (int)(theApp.m_DL.graph[2].dXMax) );
	m_scroll3.ShowWindow( TRUE );
	m_scroll4.SetScrollRange( 0, (int)(theApp.m_DL.graph[3].dXMax) );
...
void CDlgDiagLogGraph::InitGraphData()
{
	...
	theApp.m_DL.graph[0].dYMin = atof( str );
	GetDlgItem( IDC_EDIT_Graph1_Max )->GetWindowText( str );
	theApp.m_DL.graph[0].dYMax = atof( str );
	GetDlgItem( IDC_EDIT_Graph2_Min )->GetWindowText( str );
	theApp.m_DL.graph[1].dYMin = atof( str );
	GetDlgItem( IDC_EDIT_Graph2_Max )->GetWindowText( str );
	theApp.m_DL.graph[1].dYMax = atof( str );
	GetDlgItem( IDC_EDIT_Graph3_Min )->GetWindowText( str );
	theApp.m_DL.graph[2].dYMin = atof( str );
	GetDlgItem( IDC_EDIT_Graph3_Max )->GetWindowText( str );
	theApp.m_DL.graph[2].dYMax = atof( str );
	GetDlgItem( IDC_EDIT_Graph4_Min )->GetWindowText( str );
	theApp.m_DL.graph[3].dYMin = atof( str );
	GetDlgItem( IDC_EDIT_Graph4_Max )->GetWindowText( str );
	theApp.m_DL.graph[3].dYMax = atof( str );

	for( int i=0; i<GraphDataBuffGroups; i++ )
		theApp.m_DL.graph[i].dXMin = 0.0;

	m_nIntv = theApp.m_DL.other.nElapseLogging;
}
...
//	m_scroll1.SetScrollPos(m_nTotal);
	...
	for( int i=0; i<GraphDataBuffGroups; i++ )
	{
		if( theApp.m_DL.graph[i].nZoomXIndex > -1 )
		{
			theApp.m_DL.graph[i].dXMax = m_nTotal;
			theApp.m_DL.graph[i].dXMin = m_nTotal - g_dGraphRangeX[theApp.m_DL.graph[i].nZoomXIndex];
		}
	}
	CString str;
...
void CDlgDiagLogGraph::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( !(theApp.m_DL.graph[nIndex].bGraph) || (m_nTotal < 2) )
		return;
	...
	// X向绘图比例
	if( theApp.m_DL.graph[nIndex].nZoomXIndex > -1 )	// 有设置值
	{
		xMin = (int)(theApp.m_DL.graph[nIndex].dXMin);
		xMax = (int)(theApp.m_DL.graph[nIndex].dXMax);
		if( xMax > m_nTotal )
			xMax = m_nTotal;
		if( xMin >= xMax )
			xMin = xMax -2;
	...
	// 纵坐标最大值、最小值
	if( (theApp.m_DL.graph[nIndex].nZoomYIndex > -1)
		|| ((theApp.m_DL.graph[nIndex].nZoomYIndex == -1)
			&& ( (theApp.m_DL.graph[nIndex].dYMin != 0.0)
				|| (theApp.m_DL.graph[nIndex].dYMax != 0.0) ) ) )	// 如果有设置值
	{
		dYMin = theApp.m_DL.graph[nIndex].dYMin;
		dYMax = theApp.m_DL.graph[nIndex].dYMax;
	}
	else
	{
		dYMin = (theApp.m_pGraphDataBuff[nIndex])[xMin];
		dYMax = (theApp.m_pGraphDataBuff[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_pGraphDataBuff[nIndex][i] < dYMin )
				dYMin = theApp.m_pGraphDataBuff[nIndex][i];
			if( theApp.m_pGraphDataBuff[nIndex][i] > dYMax )
				dYMax = theApp.m_pGraphDataBuff[nIndex][i];
		}
	}
	/////////////////////////////////////////////////////////////////
	...
	// 画笔移到起始点
	y = (int)(rc1.top + ( nHeight - (theApp.m_pGraphDataBuff[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
	if( y < rc1.top )
		y = rc1.top;
	if( y > rc1.bottom )
		y = rc1.bottom;
	...
	for( i=xMin+1; i<xMax; i++ )
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_pGraphDataBuff[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
	...
	// 标注纵向最小值
	noldBKmode = pDC->SetBkMode(TRANSPARENT);
	double dValue = theApp.GetPhysicsFromAdcs(theApp.m_DL.graph[nIndex].nIndex, dYMin);
	str.Format( "%.6f (%.6f)", dYMin, dValue );
	pDC->TextOut( rc1.left -8, rc1.bottom -sz.cy/2, str );
	// 标注纵向最大值
	dValue = theApp.GetPhysicsFromAdcs(theApp.m_DL.graph[nIndex].nIndex, dYMax);
	str.Format( "%.6f (%.6f)", dYMax, dValue );
	pDC->TextOut( rc1.left -8, rc1.top -sz.cy/2, str );
	// 画标签
	str = theApp.m_DL.adc.chn[theApp.m_DL.graph[nIndex].nIndex].strLabel;
	sz = pDC->GetTextExtent( str );
...
/*	// TODO: Add your message handler code here
	...
	if( !(theApp.m_DL.graph[nIndex].bGraph) || (m_nTotal < 2) 
		|| (theApp.m_DL.graph[nIndex].link == NULL) )
		return;
	...
	// 找到数据链表中的用于绘图的最小值
	LinkList p = theApp.m_DL.graph[nIndex].link->next;	// 第一个数据
	for( i=0; i<xMin; i++ )
		p = p->next;
	if( p == NULL )
		return;
	...
	// 纵坐标最大值、最小值
	if( theApp.m_DL.graph[nIndex].nZoomYIndex > -1 )	// 如果有设置值
		dYMin = theApp.m_DL.graph[nIndex].dYMin;
	else
	{
		p = pMin;
...
void CDlgDiagLogGraph::OnBUTTONGraph1Setup() 
{
	...
	GetDlgItem( IDC_EDIT_Graph1_Max )->GetWindowText( str );
	theApp.m_DL.graph[0].dYMax = atof( str );
	theApp.m_DL.graph[0].nZoomYIndex = -1;
	Invalidate(FALSE);
}
...
void CDlgDiagLogGraph::OnBUTTONGraph2Setup() 
{
	...
	GetDlgItem( IDC_EDIT_Graph2_Max )->GetWindowText( str );
	theApp.m_DL.graph[1].dYMax = atof( str );
	theApp.m_DL.graph[1].nZoomYIndex = -1;
	Invalidate(FALSE);
}
...
void CDlgDiagLogGraph::OnBUTTONGraph3Setup() 
{
	...
	GetDlgItem( IDC_EDIT_Graph3_Max )->GetWindowText( str );
	theApp.m_DL.graph[2].dYMax = atof( str );
	theApp.m_DL.graph[2].nZoomYIndex = -1;
	Invalidate(FALSE);
}
...
void CDlgDiagLogGraph::OnBUTTONGraph4Setup() 
{
	...
	GetDlgItem( IDC_EDIT_Graph4_Max )->GetWindowText( str );
	theApp.m_DL.graph[3].dYMax = atof( str );
	theApp.m_DL.graph[3].nZoomYIndex = -1;
	Invalidate(FALSE);
}
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph1Y(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[0].nZoomYIndex++;
	else
		theApp.m_DL.graph[0].nZoomYIndex--;
	if( theApp.m_DL.graph[0].nZoomYIndex > g_dGraphRangeMaxIndexY )
		theApp.m_DL.graph[0].nZoomYIndex = g_dGraphRangeMaxIndexY;
	if( theApp.m_DL.graph[0].nZoomYIndex < -1 )
		theApp.m_DL.graph[0].nZoomYIndex = -1;

	int nIndex = theApp.m_DL.graph[0].nZoomYIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[0].dYMax = g_dGraphRangeY[nIndex];
		theApp.m_DL.graph[0].dYMin = -g_dGraphRangeY[nIndex];
	}
	else
	{
		theApp.m_DL.graph[0].dYMax = 0.0;
		theApp.m_DL.graph[0].dYMin = 0.0;
	}

	CString str;
	str.Format( "%.6f", theApp.m_DL.graph[0].dYMin );
	GetDlgItem( IDC_EDIT_Graph1_Min )->SetWindowText( str );
	str.Format( "%.6f", theApp.m_DL.graph[0].dYMax );
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph2Y(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[1].nZoomYIndex++;
	else
		theApp.m_DL.graph[1].nZoomYIndex--;
	if( theApp.m_DL.graph[1].nZoomYIndex > g_dGraphRangeMaxIndexY )
		theApp.m_DL.graph[1].nZoomYIndex = g_dGraphRangeMaxIndexY;
	if( theApp.m_DL.graph[1].nZoomYIndex < -1 )
		theApp.m_DL.graph[1].nZoomYIndex = -1;

	int nIndex = theApp.m_DL.graph[1].nZoomYIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[1].dYMax = g_dGraphRangeY[nIndex];
		theApp.m_DL.graph[1].dYMin = -g_dGraphRangeY[nIndex];
	}
	else
	{
		theApp.m_DL.graph[1].dYMax = 0.0;
		theApp.m_DL.graph[1].dYMin = 0.0;
	}

	CString str;
	str.Format( "%.6f", theApp.m_DL.graph[1].dYMin );
	GetDlgItem( IDC_EDIT_Graph2_Min )->SetWindowText( str );
	str.Format( "%.6f", theApp.m_DL.graph[1].dYMax );
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph3Y(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[2].nZoomYIndex++;
	else
		theApp.m_DL.graph[2].nZoomYIndex--;
	if( theApp.m_DL.graph[2].nZoomYIndex > g_dGraphRangeMaxIndexY )
		theApp.m_DL.graph[2].nZoomYIndex = g_dGraphRangeMaxIndexY;
	if( theApp.m_DL.graph[2].nZoomYIndex < -1 )
		theApp.m_DL.graph[2].nZoomYIndex = -1;

	int nIndex = theApp.m_DL.graph[2].nZoomYIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[2].dYMax = g_dGraphRangeY[nIndex];
		theApp.m_DL.graph[2].dYMin = -g_dGraphRangeY[nIndex];
	}
	else
	{
		theApp.m_DL.graph[2].dYMax = 0.0;
		theApp.m_DL.graph[2].dYMin = 0.0;
	}

	CString str;
	str.Format( "%.6f", theApp.m_DL.graph[2].dYMin );
	GetDlgItem( IDC_EDIT_Graph3_Min )->SetWindowText( str );
	str.Format( "%.6f", theApp.m_DL.graph[2].dYMax );
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph4Y(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[3].nZoomYIndex++;
	else
		theApp.m_DL.graph[3].nZoomYIndex--;
	if( theApp.m_DL.graph[3].nZoomYIndex > g_dGraphRangeMaxIndexY )
		theApp.m_DL.graph[3].nZoomYIndex = g_dGraphRangeMaxIndexY;
	if( theApp.m_DL.graph[3].nZoomYIndex < -1 )
		theApp.m_DL.graph[3].nZoomYIndex = -1;

	int nIndex = theApp.m_DL.graph[3].nZoomYIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[3].dYMax = g_dGraphRangeY[nIndex];
		theApp.m_DL.graph[3].dYMin = -g_dGraphRangeY[nIndex];
	}
	else
	{
		theApp.m_DL.graph[3].dYMax = 0.0;
		theApp.m_DL.graph[3].dYMin = 0.0;
	}

	CString str;
	str.Format( "%.6f", theApp.m_DL.graph[3].dYMin );
	GetDlgItem( IDC_EDIT_Graph4_Min )->SetWindowText( str );
	str.Format( "%.6f", theApp.m_DL.graph[3].dYMax );
...
void CDlgDiagLogGraph::OnBUTTONGraph1Default() 
{
	...
	theApp.m_DL.graph[0].nZoomXIndex = 3;	// 横向显示128个数据
	theApp.m_DL.graph[0].dXMax = g_dGraphRangeX[3];
	theApp.m_DL.graph[0].dXMin = 0;
	m_scroll1.SetScrollRange( 0, theApp.m_DL.graph[0].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[0].dXMax) : m_nTotal );
	m_scroll1.ShowWindow( TRUE );
...
void CDlgDiagLogGraph::OnBUTTONGraph2Default() 
{
	...
	theApp.m_DL.graph[1].nZoomXIndex = 3;	// 横向显示128个数据
	theApp.m_DL.graph[1].dXMax = g_dGraphRangeX[3];
	theApp.m_DL.graph[1].dXMin = 0;
	m_scroll2.SetScrollRange( 0, theApp.m_DL.graph[1].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[1].dXMax) : m_nTotal );
	m_scroll2.ShowWindow( TRUE );
...
void CDlgDiagLogGraph::OnBUTTONGraph3Default() 
{
	...
	theApp.m_DL.graph[2].nZoomXIndex = 3;	// 横向显示128个数据
	theApp.m_DL.graph[2].dXMax = g_dGraphRangeX[3];
	theApp.m_DL.graph[2].dXMin = 0;
	m_scroll3.SetScrollRange( 0, theApp.m_DL.graph[2].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[2].dXMax) : m_nTotal );
	m_scroll3.ShowWindow( TRUE );
...
void CDlgDiagLogGraph::OnBUTTONGraph4Default() 
{
	...
	theApp.m_DL.graph[3].nZoomXIndex = 3;	// 横向显示128个数据
	theApp.m_DL.graph[3].dXMax = g_dGraphRangeX[3];
	theApp.m_DL.graph[3].dXMin = 0;
	m_scroll4.SetScrollRange( 0, theApp.m_DL.graph[3].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[3].dXMax) : m_nTotal );
	m_scroll4.ShowWindow( TRUE );
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph1X(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[0].nZoomXIndex++;
	else
		theApp.m_DL.graph[0].nZoomXIndex--;
	if( theApp.m_DL.graph[0].nZoomXIndex > g_dGraphRangeMaxIndexX )
		theApp.m_DL.graph[0].nZoomXIndex = g_dGraphRangeMaxIndexX;
	if( theApp.m_DL.graph[0].nZoomXIndex < -1 )
		theApp.m_DL.graph[0].nZoomXIndex = -1;

	int nIndex = theApp.m_DL.graph[0].nZoomXIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[0].dXMax = g_dGraphRangeX[nIndex];
		theApp.m_DL.graph[0].dXMin = 0;
		m_scroll1.SetScrollRange( 0, theApp.m_DL.graph[0].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[0].dXMax) : m_nTotal );
		m_scroll1.ShowWindow( TRUE );
	}
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph2X(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[1].nZoomXIndex++;
	else
		theApp.m_DL.graph[1].nZoomXIndex--;
	if( theApp.m_DL.graph[1].nZoomXIndex > g_dGraphRangeMaxIndexX )
		theApp.m_DL.graph[1].nZoomXIndex = g_dGraphRangeMaxIndexX;
	if( theApp.m_DL.graph[1].nZoomXIndex < -1 )
		theApp.m_DL.graph[1].nZoomXIndex = -1;

	int nIndex = theApp.m_DL.graph[1].nZoomXIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[1].dXMax = g_dGraphRangeX[nIndex];
		theApp.m_DL.graph[1].dXMin = 0;
		m_scroll2.SetScrollRange( 0, theApp.m_DL.graph[1].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[1].dXMax) : m_nTotal );
		m_scroll2.ShowWindow( TRUE );
	}
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph3X(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[2].nZoomXIndex++;
	else
		theApp.m_DL.graph[2].nZoomXIndex--;
	if( theApp.m_DL.graph[2].nZoomXIndex > g_dGraphRangeMaxIndexX )
		theApp.m_DL.graph[2].nZoomXIndex = g_dGraphRangeMaxIndexX;
	if( theApp.m_DL.graph[2].nZoomXIndex < -1 )
		theApp.m_DL.graph[2].nZoomXIndex = -1;

	int nIndex = theApp.m_DL.graph[2].nZoomXIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[2].dXMax = g_dGraphRangeX[nIndex];
		theApp.m_DL.graph[2].dXMin = 0;
		m_scroll3.SetScrollRange( 0, theApp.m_DL.graph[2].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[2].dXMax) : m_nTotal );
		m_scroll3.ShowWindow( TRUE );
	}
...
void CDlgDiagLogGraph::OnDeltaposSPINGraph4X(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
	if( pNMUpDown->iDelta > 0 )
		theApp.m_DL.graph[3].nZoomXIndex++;
	else
		theApp.m_DL.graph[3].nZoomXIndex--;
	if( theApp.m_DL.graph[3].nZoomXIndex > g_dGraphRangeMaxIndexX )
		theApp.m_DL.graph[3].nZoomXIndex = g_dGraphRangeMaxIndexX;
	if( theApp.m_DL.graph[3].nZoomXIndex < -1 )
		theApp.m_DL.graph[3].nZoomXIndex = -1;

	int nIndex = theApp.m_DL.graph[3].nZoomXIndex;
	if( nIndex > -1 )
	{
		theApp.m_DL.graph[3].dXMax = g_dGraphRangeX[nIndex];
		theApp.m_DL.graph[3].dXMin = 0;
		m_scroll4.SetScrollRange( 0, theApp.m_DL.graph[3].dXMax > m_nTotal ? (int)(theApp.m_DL.graph[3].dXMax) : m_nTotal );
		m_scroll4.ShowWindow( TRUE );
	}
...
{
	theApp.m_DL.graph[0].dXMin = nPos;
	theApp.m_DL.graph[0].dXMax = nPos + g_dGraphRangeX[theApp.m_DL.graph[0].nZoomXIndex];
	Invalidate(FALSE);
}

void CDlgDiagLogGraph::Graph2XSB_Change( int nPos )
{
	theApp.m_DL.graph[1].dXMin = nPos;
	theApp.m_DL.graph[1].dXMax = nPos + g_dGraphRangeX[theApp.m_DL.graph[1].nZoomXIndex];
	Invalidate(FALSE);
}

void CDlgDiagLogGraph::Graph3XSB_Change( int nPos )
{
	theApp.m_DL.graph[2].dXMin = nPos;
	theApp.m_DL.graph[2].dXMax = nPos + g_dGraphRangeX[theApp.m_DL.graph[2].nZoomXIndex];
	Invalidate(FALSE);
}

void CDlgDiagLogGraph::Graph4XSB_Change( int nPos )
{
	theApp.m_DL.graph[3].dXMin = nPos;
	theApp.m_DL.graph[3].dXMax = nPos + g_dGraphRangeX[theApp.m_DL.graph[3].nZoomXIndex];
	Invalidate(FALSE);
}
...
void CDlgDiagLogGraph::OnBUTTONGraphSaveGraph() 
{
	...
	//画图
	CString str;
	str.Format( "nTotal = %d, Interval = %.2fs", theApp.m_DL.other.lTimes, theApp.m_DL.other.nElapseLogging / 1000.0 );
