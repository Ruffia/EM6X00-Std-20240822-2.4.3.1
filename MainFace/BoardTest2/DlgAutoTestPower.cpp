...
void CDlgAutoTestPower::InitParams()
{
	...
	lPSingle.nType	= 1;
	lPSingle.nNum	= 0;
	lPSingle.nMID	= USB_MID_SCN;
	lPSingle.nCID	= USB_CID_SCN_ADC_Read;
	lPSingle.bType	= 1;
	lPSingle.nChn	= 0;
	lPSingle.bDiff	= theApp.m_DL.adc.chn[0].bDiff;
	theApp.OnlySendReadSelectedADCcmd( lPSingle );
	theApp.OnlySendReadSelectedADCcmd( lPSingle );
	theApp.Delay( 100 );
	theApp.OnlySendReadSelectedADCcmd( lPSingle );
	theApp.Delay( 100 );
	BYTE* pbyteData = new BYTE[256];
	ZeroMemory(pbyteData, sizeof(pbyteData));
	DWORD dwReturned = 0;
	theApp.Serial_Read( pbyteData, dwReturned );
	delete [] pbyteData;
	/////////////////////////////////////////////////////////////////
...
void CDlgAutoTestPower::Start2()
{
	...
	int nRet = theApp.Serial_Read( pnFrameData, dwBytesReturned );
	if( (nRet != 0) || (dwBytesReturned < 1) )
	{
		delete [] pnFrameData;
		((CButton*)(GetDlgItem(IDC_BUTTON_AT_Cali_Power_Start)))->SetWindowText( "Start");
		((CButton*)(GetDlgItem(IDC_BUTTON_AT_Cali_Power_Start)))->EnableWindow( TRUE );
		...
		str = "Serial_Read Error!";
		((CInsample0Dlg*)(theApp.GetMainWnd()))->UpdateListReadbackString( str );
		return;
	}
	/////////////////////////////////////////////////////////////////
	...
	if( nRecvReadCounts != nSendReadCmdCounts )
	{
		...
		str.Format( "SendCounts [%d] != RecvCounts [%d]", nSendReadCmdCounts, nRecvReadCounts );
		((CInsample0Dlg*)(theApp.GetMainWnd()))->UpdateListReadbackString( str );
		return;
	}
...
BOOL CDlgAutoTestPower::StartReadAnalyze( int nMID, logParam* lP )
{
	...
	// 先看一看0xD22D的个数是否符合发出命令的个数
	/////////////////////////////////////////////////////////////////
	int i, nTotal = dwBytesReturned/2;
