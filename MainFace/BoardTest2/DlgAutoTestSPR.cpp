...
void CDlgAutoTestSPR::EnableParams( int nIndex, BOOL bEnable )
{
	if( bEnable )
	{
		...
		case	0:
			// Contrast
			SPRDACupdate( theApp.m_AT.nAutoTestSPR_OldPos[0] );
	...
	else
	{
		...
		str.Format( "%szAutoTestSPR-%s.txt", appfilepath, strTemp );
...
void CDlgAutoTestSPR::OnStartContrast() 
{
	...
	if( nCheck )
	{
		...
		theApp.m_AT.nAutoTestSPR_OldPos[0] = theApp.m_DL.diag[Diag_SPRDAC_ChA + 0].nCurPos;
		m_bContinue = TRUE;
		m_bReverse	= FALSE;
