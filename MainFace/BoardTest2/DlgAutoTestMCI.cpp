...
void CDlgAutoTestMCI::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( theApp.m_AT.nAutoTestMCI_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = theApp.m_AT.nAutoTestMCI_Total[nIndex];
	...
	{
		dYMin = (theApp.m_AT.pAutoTestMCI[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_AT.pAutoTestMCI[nIndex][i] < dYMin )
				dYMin = theApp.m_AT.pAutoTestMCI[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (theApp.m_AT.pAutoTestMCI[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_AT.pAutoTestMCI[nIndex][i] > dYMax )
				dYMax = theApp.m_AT.pAutoTestMCI[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_AT.pAutoTestMCI[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= theApp.m_AT.pAutoTestMCI[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (theApp.m_AT.pAutoTestMCI[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= theApp.m_AT.pAutoTestMCI[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
//			pDC->Ellipse( CRect(x-2, yLog-2, x+2, yLog+2) );
	...
	str.Format( "%s [%04X---%04X]", theApp.m_DL.adc.chn[m_nChnIndex2].strLabel, pn[0], pn[nMax] );
	sz = pDC->GetTextExtent( str );
...
void CDlgAutoTestMCI::EnableParams( int nIndex, BOOL bEnable )
{
	...
	else
	{
		...
		str.Format( "%szAutoTestMCI-%s.txt", appfilepath, strTemp );
...
void CDlgAutoTestMCI::OnStartFineMagX() 
{
	...
	if( nCheck )
	{
		...
		{
			theApp.m_AT.pAutoTestMCI[0][i] = 0;
			theApp.m_AT.pAutoTestMCI[1][i] = 0;
			theApp.m_AT.pAutoTestMCI[2][i] = 0;
		}
		theApp.m_AT.nAutoTestMCI_Total[0] = 0;
		theApp.m_AT.nAutoTestMCI_Total[1] = 0;
		theApp.m_AT.nAutoTestMCI_Total[2] = 0;

		theApp.m_AT.nAutoTestMCI_OldPos[0] = theApp.m_DL.diag[Diag_Stig_X].nCurPos;
		m_bContinue = TRUE;
		m_bReverse	= FALSE;
...
void CDlgAutoTestMCI::OnStartFineMagY() 
{
	...
	if( nCheck )
	{
		...
		theApp.m_AT.nAutoTestMCI_OldPos[1] = theApp.m_DL.diag[Diag_Stig_Y].nCurPos;
		m_bContinue = TRUE;
		m_bReverse	= FALSE;
...
void CDlgAutoTestMCI::OnStartShiftX() 
{
	...
	if( nCheck )
	{
		...
		theApp.m_AT.nAutoTestMCI_OldPos[2] = theApp.m_DL.diag[Diag_Shift_X].nCurPos;
		m_bContinue = TRUE;
		m_bReverse	= FALSE;
...
void CDlgAutoTestMCI::OnStartShiftY() 
{
	...
	if( nCheck )
	{
		...
		theApp.m_AT.nAutoTestMCI_OldPos[3] = theApp.m_DL.diag[Diag_Shift_Y].nCurPos;
		m_bContinue = TRUE;
		m_bReverse	= FALSE;

		// FineMag居中，Analog Shift归零，测试Digital Shift
		Det45YSB_Change( 2047 );
		YshiftDACSB_Change( 0 );
...
void CDlgAutoTestMCI::TimerChangeFineMagX( UINT nIDEvent )
{
	...
	if( nIDEvent == Timer_FineMagX_seg11 )
	{
		...
		if( theApp.m_DL.other.bDebug )
		{
			...
			nPos = theApp.m_AT.nAutoTestMCI_Total[0];
			theApp.m_AT.pAutoTestMCI[0][nPos] = dValue;
			theApp.m_AT.nAutoTestMCI_Total[0]++;
			m_strRes += str;
	...
	else if( nIDEvent == Timer_FineMagX_seg13 )
	{
		...
		if( theApp.m_DL.other.bDebug )
		{
			...
			dValue = 0.0;
			str = theApp.ReadSelectedADC_Logging_onetime( 80, dValue, TRUE );
			nPos = theApp.m_AT.nAutoTestMCI_Total[1];
			theApp.m_AT.pAutoTestMCI[1][nPos] = dValue;
			theApp.m_AT.nAutoTestMCI_Total[1]++;
			m_strRes += str;
	...
	else if( nIDEvent == Timer_FineMagX_seg15 )
	{
		...
		if( theApp.m_DL.other.bDebug )
		{
			...
			dValue = 0.0;
			str = theApp.ReadSelectedADC_Logging_onetime( 80, dValue, TRUE );
			nPos = theApp.m_AT.nAutoTestMCI_Total[2];
			theApp.m_AT.pAutoTestMCI[2][nPos] = dValue;
			theApp.m_AT.nAutoTestMCI_Total[2]++;
			m_strRes += str;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void CDlgAutoTestMCI::XstigDACSB_Change( int nPos )
{
	int nValue;
	BYTE byteHi, byteLo;
...
void CDlgAutoTestMCI::YstigDACSB_Change( int nPos )
{
	int nValue;
	BYTE byteHi, byteLo;
...
void CDlgAutoTestMCI::Det45SB_Change( int nPos )
{
	...
	WORD wValue = byteLo;
	wValue |= (((byteHi & 0xF) | (2 << 6)) << 8);
	theApp.WriteToBoard( USB_Det45StoreDAC, TRUE, wValue );
}

void CDlgAutoTestMCI::Det45YSB_Change( int nPos )
{
	// This Sub uses global long DIOData (8 hex digits) as AccesIO USB write data
	int nValue;
	BYTE byteHi, byteLo;
