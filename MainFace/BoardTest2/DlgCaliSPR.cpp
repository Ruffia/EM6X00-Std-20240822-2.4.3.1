...
void CDlgCaliSPR::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( theApp.m_Cali.m_nCaliSPR_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = theApp.m_Cali.m_nCaliSPR_Total[nIndex];
	...
	{
		dYMin = (theApp.m_Cali.m_pCaliSPR[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.m_pCaliSPR[nIndex][i] < dYMin )
				dYMin = theApp.m_Cali.m_pCaliSPR[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (theApp.m_Cali.m_pCaliSPR[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( theApp.m_Cali.m_pCaliSPR[nIndex][i] > dYMax )
				dYMax = theApp.m_Cali.m_pCaliSPR[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.m_pCaliSPR[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= theApp.m_Cali.m_pCaliSPR[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (theApp.m_Cali.m_pCaliSPR[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= theApp.m_Cali.m_pCaliSPR[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
//			pDC->Ellipse( CRect(x-2, yLog-2, x+2, yLog+2) );
	...
	// 标注纵向最小值
	noldBKmode = pDC->SetBkMode(TRANSPARENT);
	double dValue = theApp.GetPhysicsFromAdcs(5*16 +m_nChnIndex2[nIndex], dYMin);
	str.Format( "%.6f ( %.6f )", dYMin, dValue );
	pDC->TextOut( rc1.left -8, rc1.bottom -sz.cy/2, str );
	// 标注纵向最大值
	dValue = theApp.GetPhysicsFromAdcs(5*16 +m_nChnIndex2[nIndex], dYMax);
	str.Format( "%.6f ( %.6f )", dYMax, dValue );
	pDC->TextOut( rc1.left -8, rc1.top -sz.cy/2, str );
	// 画标签
	str.Format( "%s", theApp.m_DL.adc.chn[5*16 +m_nChnIndex2[nIndex]].strLabel );
	sz = pDC->GetTextExtent( str );
...
*/
	...
	for( int i=0; i<Cali_16bitDAC_Steps +1; i++ )
	{
		for( int j=0; j<1; j++ )
		{
			...
			str.Format( "%.6f", theApp.m_Cali.m_pCaliSPR[j][i]);
			sel.Move( &varOptional, &varOptional );
			sel.TypeText( str );
		}
...
void CDlgCaliSPR::InitParams()
{
	for( int i=0; i<4; i++ )
	{
		theApp.m_Cali.m_pCaliSPR[i] = new double[Cali_16bitDAC_Steps+1];
		for( int j=0; j<Cali_16bitDAC_Steps+1; j++ )
			theApp.m_Cali.m_pCaliSPR[i][j] = 0;
		theApp.m_Cali.m_nCaliSPR_Total[i] = 0;
		m_nChnIndex2[i] = 0;
	}
}
...
void CDlgCaliSPR::EnableParams( int nIndex, BOOL bEnable )
{
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= theApp.m_Cali.m_nCaliSPR_OldPos[0];
			Xpos		= var;
...
void CDlgCaliSPR::OnStartContrast() 
{
	...
	if( nCheck )
	{
		...
		for( int i=0; i<nMax; i++ )
			theApp.m_Cali.m_pCaliSPR[0][i] = 0;
		theApp.m_Cali.m_nCaliSPR_Total[0] = 0;
		theApp.m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = theApp.m_SemCtrl.m_SemCore.m_Contrast.GetPos();
		theApp.m_Cali.m_nCaliSPR_OldPos[0] = var.lVal;
...
//	if( nIDEvent == Timer_Cali_Contrast )
	{
		...
		nPos = theApp.m_Cali.m_nCaliSPR_Total[0];
		theApp.m_Cali.m_pCaliSPR[0][nPos] = dValue;
		theApp.m_Cali.m_nCaliSPR_Total[0]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
