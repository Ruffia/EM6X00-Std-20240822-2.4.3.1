// DllTest.cpp : Defines the class behaviors for the application.
//

#include "stdafx.h"
#include "DllTest.h"

#include "MainFrm.h"
#include "DllTestDoc.h"
#include "DllTestView.h"

#include "TransparentImage.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#undef THIS_FILE
static char THIS_FILE[] = __FILE__;
#endif

/////////////////////////////////////////////////////////////////////////////
// CDllTestApp

BEGIN_MESSAGE_MAP(CDllTestApp, CWinApp)
	//{{AFX_MSG_MAP(CDllTestApp)
	ON_COMMAND(ID_APP_ABOUT, OnAppAbout)
		// NOTE - the ClassWizard will add and remove mapping macros here.
		//    DO NOT EDIT what you see in these blocks of generated code!
	//}}AFX_MSG_MAP
	// Standard file based document commands
	ON_COMMAND(ID_FILE_NEW, CWinApp::OnFileNew)
	ON_COMMAND(ID_FILE_OPEN, CWinApp::OnFileOpen)
	// Standard print setup command
	ON_COMMAND(ID_FILE_PRINT_SETUP, CWinApp::OnFilePrintSetup)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDllTestApp construction

CDllTestApp::CDllTestApp()
{
	// TODO: add construction code here,
	// Place all significant initialization in InitInstance
}

/////////////////////////////////////////////////////////////////////////////
// The one and only CDllTestApp object

CDllTestApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CDllTestApp initialization

BOOL CDllTestApp::InitInstance()
{
	AfxEnableControlContainer();

	// Standard initialization
	// If you are not using these features and wish to reduce the size
	//  of your final executable, you should remove from the following
	//  the specific initialization routines you do not need.

#ifdef _AFXDLL
	Enable3dControls();			// Call this when using MFC in a shared DLL
#else
	Enable3dControlsStatic();	// Call this when linking to MFC statically
#endif

	// Change the registry key under which our settings are stored.
	// TODO: You should modify this string to be something appropriate
	// such as the name of your company or organization.
	SetRegistryKey(_T("Local AppWizard-Generated Applications"));

	LoadStdProfileSettings();  // Load standard INI file options (including MRU)

	// Register the application's document templates.  Document templates
	//  serve as the connection between documents, frame windows and views.

	CSingleDocTemplate* pDocTemplate;
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CDllTestDoc),
		RUNTIME_CLASS(CMainFrame),       // main SDI frame window
		RUNTIME_CLASS(CDllTestView));
	AddDocTemplate(pDocTemplate);

	// Parse command line for standard shell commands, DDE, file open
	CCommandLineInfo cmdInfo;
	ParseCommandLine(cmdInfo);

	// Dispatch commands specified on the command line
	if (!ProcessShellCommand(cmdInfo))
		return FALSE;

	// The one and only window has been initialized, so show and update it.
	m_pMainWnd->ShowWindow(SW_SHOW);
	m_pMainWnd->UpdateWindow();

//	ShowXYStageDlg(AfxGetMainWnd()->m_hWnd);
//	Options_XYStage_FindIndex();
	return TRUE;
}


/////////////////////////////////////////////////////////////////////////////
// CAboutDlg dialog used for App About

class CAboutDlg : public CDialog
{
	CBitmap		m_bmpON;
	CBitmap		m_bmpOFF;
public:
	CAboutDlg();
	BOOL	VAC_UpdatePneumaticDiagram();

	BOOL	m_bVf, m_bVc, m_bTMP, m_bV1, m_bVa;

// Dialog Data
	//{{AFX_DATA(CAboutDlg)
	enum { IDD = IDD_ABOUTBOX };
	CTransparentImage	m_PneumaticDiagram;
	//}}AFX_DATA

	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CAboutDlg)
	protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	//}}AFX_VIRTUAL

// Implementation
protected:
	//{{AFX_MSG(CAboutDlg)
	afx_msg void OnBUTTONPneumaticDiagramStatus();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};

CAboutDlg::CAboutDlg() : CDialog(CAboutDlg::IDD)
{
	//{{AFX_DATA_INIT(CAboutDlg)
	//}}AFX_DATA_INIT
	m_bmpON.LoadBitmap(IDB_BITMAP_LED_RED_H);
	m_bmpOFF.LoadBitmap(IDB_BITMAP_LED_RED_D);

	m_bVf	= FALSE;
	m_bVc	= FALSE;
	m_bTMP	= FALSE;
	m_bV1	= FALSE;
	m_bVa	= FALSE;
}

void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CAboutDlg)
	DDX_Control(pDX, IDC_STATIC_PneumaticDiagram, m_PneumaticDiagram);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CAboutDlg, CDialog)
	//{{AFX_MSG_MAP(CAboutDlg)
	ON_BN_CLICKED(IDC_BUTTON_PneumaticDiagram_Status, OnBUTTONPneumaticDiagramStatus)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

// App command to run the dialog
void CDllTestApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}

/////////////////////////////////////////////////////////////////////////////
// CDllTestApp message handlers


void CAboutDlg::OnBUTTONPneumaticDiagramStatus() 
{
	// TODO: Add your control notification handler code here
	static int nStatus = 0;
	nStatus++;
	if( nStatus > 5 )
		nStatus = 0;
	switch( nStatus )
	{
	case	0:
		m_bVf	= FALSE;
		m_bVc	= FALSE;
		m_bTMP	= FALSE;
		m_bV1	= FALSE;
		m_bVa	= FALSE;
		break;
	case	1:
		m_bVf	= TRUE;
		m_bVc	= FALSE;
		m_bTMP	= FALSE;
		m_bV1	= FALSE;
		m_bVa	= FALSE;
		break;
	case	2:
		m_bVf	= TRUE;
		m_bVc	= TRUE;
		m_bTMP	= FALSE;
		m_bV1	= FALSE;
		m_bVa	= FALSE;
		break;
	case	3:
		m_bVf	= TRUE;
		m_bVc	= TRUE;
		m_bTMP	= TRUE;
		m_bV1	= FALSE;
		m_bVa	= FALSE;
		break;
	case	4:
		m_bVf	= TRUE;
		m_bVc	= TRUE;
		m_bTMP	= TRUE;
		m_bV1	= TRUE;
		m_bVa	= FALSE;
		break;
	case	5:
		m_bVf	= FALSE;
		m_bVc	= FALSE;
		m_bTMP	= FALSE;
		m_bV1	= FALSE;
		m_bVa	= TRUE;
		break;
	}
	VAC_UpdatePneumaticDiagram();
}

BOOL CAboutDlg::VAC_UpdatePneumaticDiagram()
{
	// 更新气路图的各阀灯
	CStatic* pLamp = (CStatic*)GetDlgItem( IDC_STATIC_Lamp_Vacuum_Vf );
	if( pLamp != NULL )
	{
		if ( m_bVf )
			pLamp->SetBitmap((HBITMAP)(m_bmpON));
		else
			pLamp->SetBitmap((HBITMAP)(m_bmpOFF));
	}

	pLamp = (CStatic*)GetDlgItem( IDC_STATIC_Lamp_Vacuum_Vc );
	if( pLamp != NULL )
	{
		if ( m_bVc )
			pLamp->SetBitmap((HBITMAP)(m_bmpON));
		else
			pLamp->SetBitmap((HBITMAP)(m_bmpOFF));
	}

	pLamp = (CStatic*)GetDlgItem( IDC_STATIC_Lamp_Vacuum_Va );
	if( pLamp != NULL )
	{
		if ( m_bVa )
			pLamp->SetBitmap((HBITMAP)(m_bmpON));
		else
			pLamp->SetBitmap((HBITMAP)(m_bmpOFF));
	}

	pLamp = (CStatic*)GetDlgItem( IDC_STATIC_Lamp_Vacuum_TMP );
	if( pLamp != NULL )
	{
		if ( m_bTMP )
			pLamp->SetBitmap((HBITMAP)(m_bmpON));
		else
			pLamp->SetBitmap((HBITMAP)(m_bmpOFF));
	}

	pLamp = (CStatic*)GetDlgItem( IDC_STATIC_Lamp_Vacuum_V1 );
	if( pLamp != NULL )
	{
		if ( m_bV1 )
			pLamp->SetBitmap((HBITMAP)(m_bmpON));
		else
			pLamp->SetBitmap((HBITMAP)(m_bmpOFF));
	}
	return TRUE;
}
