...
//-------------------------------------------------------------------
//
void CMacButton::SetColor(COLORREF crColor)
//
// Return Value:	None.
//
...
//
{
	...
	{
		m_crDkShadow = m_crFace;//::GetSysColor(COLOR_WINDOWFRAME);
		m_crShadow = m_crFace;//::GetSysColor(COLOR_3DSHADOW);
		m_crLiteFace = LightenColor(m_crFace, 51);
		m_crLiteShadow = DarkenColor(m_crFace, 51);
	}
	else
...
//
{
	...
	if (nStyle & BS_FLAT)
	{
		COLORREF crFill = m_crFace;//::GetSysColor(COLOR_WINDOW);

		HBITMAP hBmp = GetDitherBitmap(pDC, crFill, crFill);//~crFill);
		...
		if (m_bMouseDown)
			...
			brFill.CreateSolidBrush(crFill);//~crFill);
		else
			brFill.CreateSolidBrush(DarkenColor(crFill, 30));//99));
		...
		CPen pen;
		if (nState & ODS_DISABLED)
			pen.CreatePen(PS_SOLID, 1, ~crFill);//m_crDarkDkShadow);
		else if (m_nCheck || m_bMouseDown)
			pen.CreatePen(PS_SOLID, 1, crFill);
		else
			pen.CreatePen(PS_SOLID, 1, ~crFill);
...
//
{
	...
	if (rect.PtInRect(point))
	{
		...
		// Added by Eric Hwang.
		GetParent()->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), BN_CLICKED/*m_nCheck*/), (LPARAM)GetSafeHwnd());
	}
...
{		
	if (nChar == VK_SPACE)
	{
		...
		// Send notify message to parent window.
		// Added by Eric Hwang.
		GetParent()->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), m_nCheck/*BN_CLICKED*/), (LPARAM)GetSafeHwnd());
...
//-------------------------------------------------------------------
//
void CMacRadioButton::DrawButton(LPDRAWITEMSTRUCT lpDIS)
//
// Return Value:	None.
//
// Parameters	:	lpDIS - A long pointer to a DRAWITEMSTRUCT structure.
...
//-------------------------------------------------------------------
//
void CMacRadioButton::DrawPushLikeButton(CDC *pDC, const CRect &rect, UINT nStyle, UINT nState)
//
// Return Value:	None.
//
// Parameters	:	pDC - A pointer to the DC to draw on.
...
//
{
	...
	if (nStyle & BS_FLAT)
	{
		...
		if (m_bMouseDown)
			...
			brFill.CreateSolidBrush(::GetSysColor(COLOR_BTNFACE));
...
//
{
	...
	while (pWnd && (pWnd != this))
	{
		...
		{
			if (((CMacRadioButton *)pWnd)->m_nType == TYPE_RADIO)
			{
...
//-------------------------------------------------------------------
//
void CMacRadioButton::SetColor(COLORREF crColor)
//
// Return Value:	None.
//
// Parameters	:	crColor - Used to set m_crFace.
...
LRESULT CMacRadioButton::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	if( message == WM_GETDLGCODE )
	{
		LRESULT lResult =
			CMacButton::WindowProc( message, wParam, lParam );
	...
	if( message == BM_SETCHECK )
	{
		...
		if (pWnd && (((CButton*)pWnd)->GetButtonStyle() & BS_AUTORADIOBUTTON) && ((CMacRadioButton *)pWnd)->m_nType == TYPE_RADIO)
		{
			((CMacRadioButton *)pWnd)->SetCheck(wParam);
		}
		else
			return 	CMacButton::WindowProc( message, wParam, lParam );
	} // if( message == BM_SETCHECK )
