...
int Curve::calcCurve(double *output_y)
{
	...
	double t[1024];  //array of x-coordinate of output points
	double z[1024];  //array of y-coordinate of output points
...
void Curve::SemReset(Mat &mat)
{
	clearPoints();
	addPoint(Point(0,0));
	addPoint(Point(255,255));
...
void Curves::SemReset(Mat &mat)
{
	if (CurrentChannel)  
		CurrentChannel->SemReset(mat);
}
...
uchar* Curves::adjust(InputArray src, OutputArray dst, InputArray mask)
{
	...
	if ( hasMask ) 
	{
		...
		for (int y = 0; y < height; y ++)
		{
			...
			for (int x = 0; x < width; x ++) 
			{
					...
					*out = (colorTables[*in] * pmask[x] / 255.0)
