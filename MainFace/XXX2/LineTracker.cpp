...
int CLineTracker::HitTest(CPoint point)
{
	...
	double l1 = sqrt(	pow((double)(point.x-m_rect.left), 2)+pow((double)(point.y-m_rect.top), 2));
	double l2 = sqrt(	pow((double)(point.x-m_rect.right), 2)+pow((double)(point.y-m_rect.bottom), 2));
	double l3 = sqrt(	pow((double)m_rect.Width(),  2)+pow((double)m_rect.Height(), 2) );
