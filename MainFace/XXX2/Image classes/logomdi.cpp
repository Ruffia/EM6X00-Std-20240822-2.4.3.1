...
CLogoMdi::CLogoMdi()  : m_sizeClient(0,0)
{
	...
	sLogoString = "KYKY-EM3200 ";

	//BITMAP background
	HBITMAP bitmap;
	if (bitmap = ::LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_3K2_BITMAP_Wallpaper))){
		bgImage = (CxImage *) new CxImageBMP();
		bgImage->CreateFromHBITMAP( (HBITMAP)bitmap );
	} else {
		bgImage = NULL;
	}
