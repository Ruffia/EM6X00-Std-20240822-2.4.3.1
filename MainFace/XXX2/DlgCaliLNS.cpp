...
void CDlgCaliLNS::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( CControlLayer::Instance().m_Cali.nCaliLNS_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = CControlLayer::Instance().m_Cali.nCaliLNS_Total[nIndex];
	...
	{
		dYMin = (CControlLayer::Instance().m_Cali.pCaliLNS[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][i] < dYMin )
				dYMin = CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (CControlLayer::Instance().m_Cali.pCaliLNS[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][i] > dYMax )
				dYMax = CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= CControlLayer::Instance().m_Cali.pCaliLNS[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
//			pDC->Ellipse( CRect(x-2, yLog-2, x+2, yLog+2) );
	...
	// 标注纵向最小值
	noldBKmode = pDC->SetBkMode(TRANSPARENT);
	double dValue = CBoardComm::Instance().GetPhysicsFromAdcs(1*16 +m_nChnIndex2[nIndex], dYMin, CControlLayer::Instance().m_DL.log.strCalc);
	str.Format( "%.6f ( %.6f )", dYMin, dValue );
	pDC->TextOut( rc1.left -8, rc1.bottom -sz.cy/2, str );
	// 标注纵向最大值
	dValue = CBoardComm::Instance().GetPhysicsFromAdcs(1*16 +m_nChnIndex2[nIndex], dYMax, CControlLayer::Instance().m_DL.log.strCalc);
	str.Format( "%.6f ( %.6f )", dYMax, dValue );
	pDC->TextOut( rc1.left -8, rc1.top -sz.cy/2, str );
	// 画标签
	str.Format( "%s", CControlLayer::Instance().m_DL.adc.chn[1*16 +m_nChnIndex2[nIndex]].strLabel );
	sz = pDC->GetTextExtent( str );
...
void CDlgCaliLNS::Report( CString filename ) 
{
	...
	for( int i=0; i<Cali_16bitDAC_Steps +1; i++ )
	{
		for( int j=0; j<4; j++ )
		{
			...
			str.Format( "%.6f", CControlLayer::Instance().m_Cali.pCaliLNS[j][i]);
			sel.Move( &varOptional, &varOptional );
			sel.TypeText( str );
		}
...
void CDlgCaliLNS::InitParams()
{
	for( int i=0; i<4; i++ )
	{
		CControlLayer::Instance().m_Cali.pCaliLNS[i] = new double[Cali_16bitDAC_Steps+1];
		for( int j=0; j<Cali_16bitDAC_Steps+1; j++ )
			CControlLayer::Instance().m_Cali.pCaliLNS[i][j] = 0;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[i] = 0;
		m_nChnIndex2[i] = 0;
	}
}
...
void CDlgCaliLNS::EnableParams( int nIndex, BOOL bEnable )
{
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[0];
			Xpos		= var;
		...
		case	2:
			...
			Flag.lVal	= 1;
			var.dblVal	= CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[1];
		...
		case	4:
			...
			Flag.lVal	= 1;
			var.lVal	= CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[2];
			Xpos		= var;
		...
		case	6:
			...
			ClassID.lVal = SemControlPanelClass_BeemAlignment;
			Flag.lVal	= 2;
			var.lVal	= CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[3];
			Ypos		= var;
...
void CDlgCaliLNS::OnStartCL() 
{
	...
	if( nCheck )
	{
		...
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliLNS[0][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[0] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_CondenserLens.GetPos();
		CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[0] = var.lVal;
...
void CDlgCaliLNS::OnStartOL() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliLNS[1][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[1] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_ObjectiveLens.GetPos();
		CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[1] = var.lVal;
...
void CDlgCaliLNS::OnStartALX() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliLNS[2][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[2] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_BeemAlignmentX.GetPos();
		CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[2] = var.lVal;
...
void CDlgCaliLNS::OnStartALY() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliLNS[3][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[3] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_BeemAlignmentY.GetPos();
		CControlLayer::Instance().m_Cali.nCaliLNS_OldPos[3] = var.lVal;
...
//	if( nIDEvent == Timer_Cali_CL )
	{
		...
		nPos = CControlLayer::Instance().m_Cali.nCaliLNS_Total[0];
		CControlLayer::Instance().m_Cali.pCaliLNS[0][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[0]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
...
//	if( nIDEvent == Timer_Cali_OL )
	{
		...
		dValue = 0.0;
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 5, dValue , CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log );
		nPos = CControlLayer::Instance().m_Cali.nCaliLNS_Total[1];
		CControlLayer::Instance().m_Cali.pCaliLNS[1][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[1]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( CControlLayer::Instance().m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_ALX )
	{
		...
		dValue = 0.0;
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 5, dValue , CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log );
		nPos = CControlLayer::Instance().m_Cali.nCaliLNS_Total[2];
		CControlLayer::Instance().m_Cali.pCaliLNS[2][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[2]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( CControlLayer::Instance().m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_ALY )
	{
		...
		dValue = 0.0;
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 5, dValue , CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log );
		nPos = CControlLayer::Instance().m_Cali.nCaliLNS_Total[3];
		CControlLayer::Instance().m_Cali.pCaliLNS[3][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliLNS_Total[3]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( CControlLayer::Instance().m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
