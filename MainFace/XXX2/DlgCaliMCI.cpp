...
void CDlgCaliMCI::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( CControlLayer::Instance().m_Cali.nCaliMCI_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = CControlLayer::Instance().m_Cali.nCaliMCI_Total[nIndex];
	...
	{
		dYMin = (CControlLayer::Instance().m_Cali.pCaliMCI[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][i] < dYMin )
				dYMin = CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (CControlLayer::Instance().m_Cali.pCaliMCI[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][i] > dYMax )
				dYMax = CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= CControlLayer::Instance().m_Cali.pCaliMCI[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
/*	for( int i=0; i<Cali_16bitDAC_Steps +1; i++ )
	{
		for( int j=0; j<4; j++ )
		{
			...
			str.Format( "%.6f", CControlLayer::Instance().m_Cali.pCaliMCI[j][i]);
			sel.Move( &varOptional, &varOptional );
			sel.TypeText( str );
		}
...
void CDlgCaliMCI::InitParams()
{
	for( int i=0; i<4; i++ )
	{
		CControlLayer::Instance().m_Cali.pCaliMCI[i] = new double[Cali_16bitDAC_Steps+1];
		for( int j=0; j<Cali_16bitDAC_Steps+1; j++ )
			CControlLayer::Instance().m_Cali.pCaliMCI[i][j] = 0;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[i] = 0;
		m_nChnIndex2[i] = 0;
	}
}
...
void CDlgCaliMCI::EnableParams( int nIndex, BOOL bEnable )
{
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[0];
			Xpos		= var;
		...
		case	2:
			...
			ClassID.lVal = SemControlPanelClass_Stigmator;
			Flag.lVal	= 2;
			var.lVal	= CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[1];
			Ypos		= var;
		...
		case	4:
			...
			Flag.lVal	= 1;
			var.lVal	= CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[2];
			Xpos		= var;
		...
		case	6:
			...
			ClassID.lVal = SemControlPanelClass_ScanShift;
			Flag.lVal	= 2;
			var.lVal	= CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[3];
			Ypos		= var;
...
void CDlgCaliMCI::OnStartFineMagX() 
{
	...
	if( nCheck )
	{
		...
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliMCI[0][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[0] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_StigmatorX.GetPos();
		CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[0] = var.lVal;

		// Analog Shift归零，Digital Shift归零，测试FineMag
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelSL.m_scrollXshiftDAC.SetScrollPos( 0 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelSL.XshiftDACSB_Change( 0 );
		CSemCtrl::Instance().USB_WriteToBoard( USB_SetXoffset, TRUE, 0 );	// Digital Shift X

		int nPos = 0;	// 设置起始值
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.SelchangePage( 2 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_nDet45Chn2 = 2;
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.GetDlgItem( IDC_EDIT_Det45_Chn2 )->SetWindowText( "2" );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_scrollDet45.SetScrollPos( nPos );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.Det45SB_Change( nPos );
...
void CDlgCaliMCI::OnStartFineMagY() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_12bitDAC_Steps;
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliMCI[1][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[1] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_StigmatorY.GetPos();
		CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[1] = var.lVal;

		// Analog Shift归零，Digital Shift归零，测试FineMag
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelSL.m_scrollYshiftDAC.SetScrollPos( 0 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelSL.YshiftDACSB_Change( 0 );
		CSemCtrl::Instance().USB_WriteToBoard( USB_SetYoffset, TRUE, 0 );	// Digital Shift Y

		int nPos = 0;	// 设置起始值
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.SelchangePage( 2 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_nDet45Chn2 = 3;
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.GetDlgItem( IDC_EDIT_Det45_Chn2 )->SetWindowText( "3" );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_scrollDet45.SetScrollPos( nPos );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.Det45SB_Change( nPos );
...
void CDlgCaliMCI::OnStartShiftX() 
{
	...
	if( nCheck )
	{
		...
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliMCI[2][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[2] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_ScanShiftX.GetPos();
		CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[2] = var.lVal;

		// FineMag居中，Analog Shift归零，测试Digital Shift
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_nDet45Chn2 = 2;
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.GetDlgItem( IDC_EDIT_Det45_Chn2 )->SetWindowText( "2" );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_scrollDet45.SetScrollPos( 2047 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.Det45SB_Change( 2047 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelSL.m_scrollXshiftDAC.SetScrollPos( 0 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelSL.XshiftDACSB_Change( 0 );

		int nPos = -32768;	// 设置起始值
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.SelchangePage( 2 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_scrollSegDShiftX.SetScrollPos( nPos );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.SegDShiftXSB_Change( nPos );
...
void CDlgCaliMCI::OnStartShiftY() 
{
	...
	if( nCheck )
	{
		...
		int nMax = Cali_16bitDAC_Steps;
		for( int i=0; i<nMax; i++ )
			CControlLayer::Instance().m_Cali.pCaliMCI[3][i] = 0;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[3] = 0;
		CControlLayer::Instance().m_DL.adc.nAveCounts	= Cali_Ave_Counts;

		VARIANT var = CSemCoreAssemble::Instance().m_ScanShiftY.GetPos();
		CControlLayer::Instance().m_Cali.nCaliMCI_OldPos[3] = var.lVal;
		...
		int nPos = -32768;	// 设置起始值
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.SelchangePage( 2 );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.m_scrollSegDShiftY.SetScrollPos( nPos );
		((CMainFrame *)theApp.GetMainWnd())->m_MainPanelDebug.m_PagePanelMCI.SegDShiftYSB_Change( nPos );
...
//	if( nIDEvent == Timer_Cali_FineMagX )
	{
		...
		nPos = CControlLayer::Instance().m_Cali.nCaliMCI_Total[0];
		CControlLayer::Instance().m_Cali.pCaliMCI[0][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[0]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		...
		if( CControlLayer::Instance().m_DL.diag[m_nChnIndex1].nCurPos == g_n12bitDAC_Max )
		{
...
//	if( nIDEvent == Timer_Cali_FineMagY )
	{
		...
		dValue = 0.0;
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 80, dValue, CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log  );
		nPos = CControlLayer::Instance().m_Cali.nCaliMCI_Total[1];
		CControlLayer::Instance().m_Cali.pCaliMCI[1][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[1]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( CControlLayer::Instance().m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_MCI_ShiftX )
	{
		...
		dValue = 0.0;
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 80, dValue , CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log );
		nPos = CControlLayer::Instance().m_Cali.nCaliMCI_Total[2];
		CControlLayer::Instance().m_Cali.pCaliMCI[2][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[2]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( CControlLayer::Instance().m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
...
//	if( nIDEvent == Timer_Cali_MCI_ShiftY )
	{
		...
		dValue = 0.0;
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 80, dValue , CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log );
		nPos = CControlLayer::Instance().m_Cali.nCaliMCI_Total[3];
		CControlLayer::Instance().m_Cali.pCaliMCI[3][nPos] = dValue;
		CControlLayer::Instance().m_Cali.nCaliMCI_Total[3]++;
		m_strRes += str;
		// 记录数值到文件
		sprintf( Buf, "%s\n", m_strRes );
		WriteFile( CControlLayer::Instance().m_Cali.hLogFile, Buf, strlen(Buf), &cbRet, NULL );
