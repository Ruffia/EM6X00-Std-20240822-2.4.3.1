...
HBRUSH CDlgAutoTestPower::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	if( nCtlColor == CTLCOLOR_STATIC )
	{
		...
		int nIndex1 = nID - IDC_STATIC_AT_Cali_Power_SCN10;
  		int nIndex2 = nID - IDC_STATIC_AT_Cali_Power_MCI0; // MCI
		if( (nIndex1 >=0 && nIndex1 <= 35
				&& CControlLayer::Instance().m_AT.pPower_OutofRange[nIndex1])
		   || (nIndex2 >=0 && nIndex2 <=7
				&& CControlLayer::Instance().m_AT.pPower_OutofRange[33+nIndex2]) )
		{
...
void CDlgAutoTestPower::InitParams()
{
	...
	// SCN ADC:
	for(int i=10; i<16; i++ )
		GetDlgItem( IDC_STATIC_AT_Cali_Power_SCN10 +i -10)->SetWindowText( CControlLayer::Instance().m_DL.adc.chn[0*16+i].strLabel );
	/////////////////////////////////////////////////////////////////
	
	/////////////////////////////////////////////////////////////////
	// LNS ADC:
	for(int i=4; i<8; i++ )
		GetDlgItem( IDC_STATIC_AT_Cali_Power_LNS4 +i -4)->SetWindowText(CControlLayer::Instance().m_DL.adc.chn[1*16+i].strLabel );
	/////////////////////////////////////////////////////////////////
	
	/////////////////////////////////////////////////////////////////
	// VAC ADC#1:
	for(int i=10; i<16; i++ )
		GetDlgItem( IDC_STATIC_AT_Cali_Power_VAC10 +i -10)->SetWindowText( CControlLayer::Instance().m_DL.adc.chn[3*16+i].strLabel );
	/////////////////////////////////////////////////////////////////
	
	/////////////////////////////////////////////////////////////////
	// SPR ADC#0:
	for(int i=0; i<15; i++ )
		GetDlgItem( IDC_STATIC_AT_Cali_Power_SPR0 +i)->SetWindowText( CControlLayer::Instance().m_DL.adc.chn[4*16+i].strLabel );
	/////////////////////////////////////////////////////////////////
	
	/////////////////////////////////////////////////////////////////
	// SPR ADC#1:
	for(int i=5; i<8; i++ )
		GetDlgItem( IDC_STATIC_AT_Cali_Power_SPR16 +i -5)->SetWindowText( CControlLayer::Instance().m_DL.adc.chn[5*16+i].strLabel );
	/////////////////////////////////////////////////////////////////
	...
	for(int i=0; i<33; i++ )
	{
		if( (i == 6+4+6 +7) || (i == 6+4+6+14) )
			l++;
		str.Format( "%.3f", CControlLayer::Instance().m_AT.pAutoTestPowerExp[i] );
	...
	for(int i=0; i<8; i++ )
	{
		...
		str.Format( "%.3f", CControlLayer::Instance().m_AT.pAutoTestPowerExp[33+i] );
...
void CDlgAutoTestPower::Start1()
{
	...
	for( i=10; i<16; i++ )
	{
		...
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 100, dValue , CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log );
		strRes += str;
		CControlLayer::Instance().m_AT.pAutoTestPower[k] = dValue;
	...
	for( i=0; i<16; i++ )
	{
		...
		theApp.Delay(10);
		str = CBoardComm::Instance().ReadSelectedADC_Logging2( 120, dValue , CControlLayer::Instance().m_DL.adc, CControlLayer::Instance().m_DL.log );
		strRes += str;
		CControlLayer::Instance().m_AT.pAutoTestPower[nIndex +j] = dValue;
		str.Format( "%.6f", dValue );
	...
	for( i=10; i<16; i++ )
	{
		dValue = CBoardComm::Instance().GetPhysicsFromAdcs( j*16 +i, CControlLayer::Instance().m_AT.pAutoTestPower[k] , CControlLayer::Instance().m_DL.log.strCalc);
		str.Format( "%.6f", dValue );
...
void CDlgAutoTestPower::Start2()
{
	...
	for( i=10; i<16; i++ )
	{
		...
		lP[nTotalADCs].bDiff	= CControlLayer::Instance().m_DL.adc.chn[lP[nTotalADCs].nNum*16 +i].bDiff;
		CBoardComm::Instance().OnlySendReadSelectedADCcmd( lP[nTotalADCs] );
		CBoardComm::Instance().OnlySendReadSelectedADCcmd( lP[nTotalADCs] );
		theApp.Delay( 100 );
		CBoardComm::Instance().OnlySendReadSelectedADCcmd( lP[nTotalADCs] );
		theApp.Delay( 100 );
	...
	for( i=0; i<nTotal; i++ )
	{
		if( (i == 6+4+6 +7) || (i == 6+4+6 +14) )
			l++;
		str = CBoardComm::Instance().ADCvconv_Logging( pnValidData[i*2], pnValidData[i*2+1], lP[i], dValue , CControlLayer::Instance().m_DL.adc.chn);
		strRes += str;
		CControlLayer::Instance().m_AT.pAutoTestPower[k] = dValue;
		str.Format( "%.6f", dValue );
	...
	for( i=0; i<nTotal; i++ )
	{
		if( (i == 6+4+6 +7) || (i == 6+4+6+14) )
			l++;
		if( (CControlLayer::Instance().m_AT.pAutoTestPowerExp[i]) != 0 && (CControlLayer::Instance().m_AT.pAutoTestPower[i] != 0) )
		{
			dErr = fabs(CControlLayer::Instance().m_AT.pAutoTestPower[i] -CControlLayer::Instance().m_AT.pAutoTestPowerExp[i]) / fabs(CControlLayer::Instance().m_AT.pAutoTestPowerExp[i]);
			str.Format( "%.2f %%", dErr *100 );
			GetDlgItem( IDC_EDIT_AT_Cali_Power_SCN10_Vadc4 + l )->SetWindowText( str );
			if( dErr *100 > CControlLayer::Instance().m_AT.pAutoTestPowerErr[i] )
				CControlLayer::Instance().m_AT.pPower_OutofRange[l] = TRUE;
		}
		l++;
	}
...
BOOL CDlgAutoTestPower::StartReadAnalyze( int nMID, logParam* lP )
{
	...
	for( i=0; i<nTotal; i++ )
	{
		...
		str = CBoardComm::Instance().ADCvconv_Logging( pnValidData[i*2], pnValidData[i*2+1], lP[i], dValue,CControlLayer::Instance().m_DL.adc.chn);
		strRes += str;
		CControlLayer::Instance().m_AT.pAutoTestPower[nPwrArrayIndex] = dValue;
		str.Format( "%.6f", dValue );
	...
	for( i=0; i<nTotal; i++ )
	{
		if( (nMID == 4) && ((i == 7) || (i == 14)) )
			nLabelIndex++;
		if( (CControlLayer::Instance().m_AT.pAutoTestPowerExp[nPwrArrayIndex]) != 0 && (CControlLayer::Instance().m_AT.pAutoTestPower[nPwrArrayIndex] != 0) )
		{
			dErr = fabs(CControlLayer::Instance().m_AT.pAutoTestPower[nPwrArrayIndex] -CControlLayer::Instance().m_AT.pAutoTestPowerExp[nPwrArrayIndex]) / fabs(CControlLayer::Instance().m_AT.pAutoTestPowerExp[nPwrArrayIndex]);
			str.Format( "%.2f %%", dErr *100 );
			GetDlgItem( IDC_EDIT_AT_Cali_Power_SCN10_Vadc4 + nLabelIndex )->SetWindowText( str );
			if( dErr *100 > CControlLayer::Instance().m_AT.pAutoTestPowerErr[nPwrArrayIndex] )
				CControlLayer::Instance().m_AT.pPower_OutofRange[nLabelIndex] = TRUE;
		}
		nPwrArrayIndex++;
		nLabelIndex++;
	}
	...
	if( nMID == 4 )
	{
		...
		for( i=0; i<15; i++ )
		{
			...
			dValue = CBoardComm::Instance().GetPhysicsFromAdcs( nNum*16 +i, CControlLayer::Instance().m_AT.pAutoTestPower[nPwrArrayIndex] , CControlLayer::Instance().m_DL.log.strCalc);
			str.Format( "%.6f", dValue );
...
BOOL CDlgAutoTestPower::StartReadAnalyze( unsigned short* pPwr )	// MCI Pwr
{
	...
	for(int i=0; i<8; i++ )
	{
		if( (i == 3) || (i == 4) )
			continue;
		nPwrArrayIndex	= 33+i;
		str.Format( "%.3f", CControlLayer::Instance().m_AT.pAutoTestPower[33+i] );
	...
	for(int i=0; i<8; i++ )
	{
		...
		if( (CControlLayer::Instance().m_AT.pAutoTestPowerExp[nPwrArrayIndex]) != 0 && (CControlLayer::Instance().m_AT.pAutoTestPower[nPwrArrayIndex] != 0) )
		{
			...
			if( dErr *100 > CControlLayer::Instance().m_AT.pAutoTestPowerErr[nPwrArrayIndex] )
				CControlLayer::Instance().m_AT.pPower_OutofRange[33+nLabelIndex] = TRUE;
		}
//		nPwrArrayIndex++;
		nLabelIndex++;
	}
...
void CDlgAutoTestPower::PwrVconV( unsigned short nPwr )
{
	...
	case	2:
		...
		break;
	case	1:
		dValue = 3.5 *g_dPwrbitsRange *wData /g_wPwrbitsRange - 2.5 *CControlLayer::Instance().m_AT.pAutoTestPower[33+2];
		strRef = "-8V";
		break;
	case	0:
		dValue = 3.5 *g_dPwrbitsRange *wData /g_wPwrbitsRange - 2.5 *CControlLayer::Instance().m_AT.pAutoTestPower[33+5];
		strRef = "-5V";
		break;
	default:
		break;
	}
	CControlLayer::Instance().m_AT.pAutoTestPower[33+nChn] = dValue;
}
...
//		GetDlgItem( IDC_EDIT_AT_Cali_Power_SCN10_Vadc2 + i )->SetWindowText( "" );
	...
	{
		CControlLayer::Instance().m_AT.pAutoTestPower[i] = 0.0;
		CControlLayer::Instance().m_AT.pPower_OutofRange[i] = FALSE;
	}
	Invalidate();
}
