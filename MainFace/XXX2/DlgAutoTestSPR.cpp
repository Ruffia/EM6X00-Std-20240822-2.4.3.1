...
void CDlgAutoTestSPR::PaintGraph( CDC* pDC, int nIndex, BOOL bErase )
{
	...
	if( CControlLayer::Instance().m_AT.nAutoTestSPR_Total[nIndex] < 2 )
		return;
	...
	rc1.bottom	-= 20;
	nHeight = rc1.Height();
	nTotal = CControlLayer::Instance().m_AT.nAutoTestSPR_Total[nIndex];
	...
	{
		dYMin = (CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][i] < dYMin )
				dYMin = CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][i];
		}
//		dYMinLog = log(dYMin) / log(2);
	}
	{
		dYMax = (CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex])[xMin];
		for( i=xMin+1; i<xMax; i++ )
		{
			if( CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][i] > dYMax )
				dYMax = CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][i];
		}
...
//		yLog = rc1.bottom;
	...
	{
		y = (int)(rc1.top + ( nHeight - (CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][xMin] -dYMin) / (dYMax-dYMin) * nHeight ));
//		dY		= CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][xMin];
...
//			yLog = rc1.bottom;
		...
		{
			y = (int)(rc1.top + ( nHeight - (CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][i] -dYMin) / (dYMax-dYMin) * nHeight ));
//			dY		= CControlLayer::Instance().m_AT.pAutoTestSPR[nIndex][i];
//			yLog	= (int)(rc1.top + ( nHeight - ( log(dY) / log(2) -dYMinLog) / (dYMaxLog-dYMinLog) * nHeight ));
		}
...
//			pDC->Ellipse( CRect(x-2, yLog-2, x+2, yLog+2) );
	...
	// 标注纵向最小值
	noldBKmode = pDC->SetBkMode(TRANSPARENT);
	double dValue = CBoardComm::Instance().GetPhysicsFromAdcs(5*16 +m_nChnIndex2, dYMin, CControlLayer::Instance().m_DL.log.strCalc);
	str.Format( "%.6f ( %.6f )", dYMin, dValue );
	pDC->TextOut( rc1.left -8, rc1.bottom -sz.cy/2, str );
	// 标注纵向最大值
	dValue = CBoardComm::Instance().GetPhysicsFromAdcs(5*16 +m_nChnIndex2, dYMax, CControlLayer::Instance().m_DL.log.strCalc);
	...
	str.Format( "%s [%04X---%04X]", CControlLayer::Instance().m_DL.adc.chn[5*16 +m_nChnIndex2].strLabel, pn[0], pn[nMax] );
	sz = pDC->GetTextExtent( str );
...
void CDlgAutoTestSPR::InitParams()
{
	for( int i=0; i<4; i++ )
	{
		CControlLayer::Instance().m_AT.pAutoTestSPR[i] = new double[AutoTest_16bitDAC_Steps*3];
		CControlLayer::Instance().m_AT.nAutoTestSPR_Total[i] = 0;
	}
}
...
void CDlgAutoTestSPR::EnableParams( int nIndex, BOOL bEnable )
{
	if( bEnable )
	{
		...
		case	0:
			...
			Flag.lVal	= 1;
			var.lVal	= CControlLayer::Instance().m_AT.nAutoTestSPR_OldPos[0];
			Xpos		= var;
...
void CDlgAutoTestSPR::OnStartContrast() 
{
	...
	if( nCheck )
	{
		...
		{
			CControlLayer::Instance().m_AT.pAutoTestSPR[0][i] = 0;
			CControlLayer::Instance().m_AT.pAutoTestSPR[1][i] = 0;
			CControlLayer::Instance().m_AT.pAutoTestSPR[2][i] = 0;
		}
		CControlLayer::Instance().m_AT.nAutoTestSPR_Total[0] = 0;
		CControlLayer::Instance().m_AT.nAutoTestSPR_Total[1] = 0;
		CControlLayer::Instance().m_AT.nAutoTestSPR_Total[2] = 0;

		VARIANT var = CSemCoreAssemble::Instance().m_Contrast.GetPos();
		CControlLayer::Instance().m_AT.nAutoTestSPR_OldPos[0] = var.lVal;
		m_bContinue = TRUE;
		m_bReverse	= FALSE;
...
void CDlgAutoTestSPR::TimerChangeContrast( UINT nIDEvent )
{
	...
	if( nIDEvent == Timer_Contrast_seg11 )
	{
		...
		if( CControlLayer::Instance().m_DL.other.bDebug )
		{
			...
			nPos = CControlLayer::Instance().m_AT.nAutoTestSPR_Total[0];
			CControlLayer::Instance().m_AT.pAutoTestSPR[0][nPos] = dValue;
			CControlLayer::Instance().m_AT.nAutoTestSPR_Total[0]++;
			m_strRes += str;
	...
	else if( nIDEvent == Timer_Contrast_seg13 )
	{
		...
		if( CControlLayer::Instance().m_DL.other.bDebug )
		{
			...
			dValue = 0.0;
			str = CBoardComm::Instance().ReadSelectedADC_Logging_onetime( 80, dValue, CControlLayer::Instance().m_DL.adc.chn, TRUE );
			nPos = CControlLayer::Instance().m_AT.nAutoTestSPR_Total[1];
			CControlLayer::Instance().m_AT.pAutoTestSPR[1][nPos] = dValue;
			CControlLayer::Instance().m_AT.nAutoTestSPR_Total[1]++;
			m_strRes += str;
	...
	else if( nIDEvent == Timer_Contrast_seg15 )
	{
		...
		if( CControlLayer::Instance().m_DL.other.bDebug )
		{
			...
			dValue = 0.0;
			str = CBoardComm::Instance().ReadSelectedADC_Logging_onetime( 80, dValue, CControlLayer::Instance().m_DL.adc.chn, TRUE );
			nPos = CControlLayer::Instance().m_AT.nAutoTestSPR_Total[2];
			CControlLayer::Instance().m_AT.pAutoTestSPR[2][nPos] = dValue;
			CControlLayer::Instance().m_AT.nAutoTestSPR_Total[2]++;
			m_strRes += str;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void CDlgAutoTestSPR::SPRDACupdate( int nPos )
{
