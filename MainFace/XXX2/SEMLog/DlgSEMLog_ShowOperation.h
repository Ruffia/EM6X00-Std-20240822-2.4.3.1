...
class CDlgSEMLog_ShowOperation : public CDlgGridPage
{
	DECLARE_DYNAMIC(CDlgSEMLog_ShowOperation)

public:
	CDlgSEMLog_ShowOperation(CWnd* pParent = NULL);   // 标准构造函数
	virtual ~CDlgSEMLog_ShowOperation();
	void	Localize(void);

// 对话框数据
	enum { IDD = IDD_DLG_SEMLog_Operation };
...
protected:
	...
	void _QueryData(const string strFieldSelect, const string sTableName, const string strKeyWord);
