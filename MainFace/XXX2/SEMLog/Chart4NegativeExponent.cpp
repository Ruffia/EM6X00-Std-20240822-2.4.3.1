...
CChart4NegativeExponent::CChart4NegativeExponent()
{
	m_bSelected = false;
}

bool CChart4NegativeExponent::DrawChartCurve(V_CHARTDATAD& vDataPntsD, GraphicsPath* grPathPtr, Graphics* grPtr, float dpiRatio)
{
	...
	if (m_pAxis->m_vecGridXGraduation.size() <= 0) { return false;}
	if (m_pAxis->m_vecGridYGraduation.size() <= 0) { return false;}
	...
	bool bConvertChartData = ConvertChartData(vDataPntsD, vDataPntsF, m_pAxis->m_pMatrixD, startX, endX);
	if (!bConvertChartData) 
		return false;
...
bool CChart4NegativeExponent::ConvertChartData(V_CHARTDATAD& vDataPnts, V_CHARTDATAF& vDataPntsF, MatrixD* pMatrixD, double startX, double endX)
{
