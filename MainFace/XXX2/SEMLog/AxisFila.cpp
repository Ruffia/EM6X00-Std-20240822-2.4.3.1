...
CAxisFila::CAxisFila()
{
	m_bDrawXAxis = FALSE;
	m_fOffSetYAxis = 50.f;
	m_colAxis = ((ARGB)Color::Blue);
	m_colGrid = ((ARGB)Color::BlueViolet);
}
...
// Draw functions
PointF CAxisFila::DrawAxes(double startX, double endX, double minY, 
...
void CAxisFila::DrawGrid(PointF coordOrigF, Graphics* grPtr, float dpiRatio)
{
	USES_CONVERSION;
	...
	while (yF > limTop && i >= 0)
	{
		if (0 == i%5)
		{
			...
			TCHAR sz[16] = {0};
			_sntprintf(sz,16,_T("%.1f"),m_vecGridYGraduation[i]);
			string_t strX = sz;
			PointF PointX;
			PointX.X = limLeft - 30;
			PointX.Y = yF - 5;
	...
	if (m_bVisible)
	{
		...
		grPtr->DrawString(A2W("Fila(A)"),-1, &axFont,PointX, &axLabBr);
	}
