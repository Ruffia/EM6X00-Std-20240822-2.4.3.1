...
// Draw functions
PointF CAxisHV::DrawAxes(double startX, double endX, double minY, 
								 double maxY, Graphics* grPtr, float dpiRatio)
{
	Pen axisPen(m_colAxis, AXIS_PENWIDTH*dpiRatio);  

	m_fOffSetYAxis = -(m_drawRF.Width + 80);
...
void CAxisHV::DrawGrid(PointF coordOrigF, Graphics* grPtr, float dpiRatio)
{
	USES_CONVERSION;
	...
	if (m_bVisible)
	{
		...
		grPtr->DrawString(A2W("HV(KV)"),-1, &axFont,PointX, &axLabBr);
	}
