...
begin
    KYKYCtrlInit(Handle);
//    KYKYCtrlConnect(30,0,0,1); // IP address
    KYKYCtrlConnect(192,168,1,58); // IP address
end;
...
var
	...
	ParamL: NativeINT;
  nS, nX, nY: Integer;
  fWD, fB, fC, fM, fX, fY, fR, fS: Single;
begin
	ParamW:=msg.wParam;
	ParamL:=msg.lParam;
  ...
  begin
    case ParamW of
      ...
      begin
        ...
        // WD, Brightness, Contrast, Mag, Stig
        KYKYCtrlGetWD( @fWD );
        edtWD.Text:=Format('%.1f', [fWD]);
        KYKYCtrlGetBrightness( @fB );
        edtBrt.Text:=Format('%.2f', [fB]);
        ...
        if( nS = 0 ) then
          ...
          btnSetBeam.Enabled:=True;
        KYKYCtrlGetAccVoltage( @fS );
        edtHV.Text:=Format('%.1f', [fS]);
        KYKYCtrlGetFilament( @fS );
        edtFilament.Text:=Format('%.2f', [fS]);
        KYKYCtrlGetBeamCurrent( @fS );
        edtBeam.Text:=Format('%.1f', [fS]);
        // Stage
        KYKYCtrlGetStageStatus( @nS );
        m_nStatusStage:=nS;
        edtStageStatus.Text:=Format('0x%02X', [nS]);
        if( (nS & 0x03) = 0x01 ) then
        begin
          KYKYCtrlGetStageGratings( @nX, @nY );
          edtStageGrating.Text:=Format('%d,%d', [nX, nY]);
          KYKYCtrlGetStageRanges( @nX, @nY );
